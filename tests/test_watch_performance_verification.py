...
class TestWatchPerformanceVerification(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_poller_uses_bulk_update(self):
        ...
        # Setup mocks
        redis = AsyncMock() # Poller uses async methods on manager which might use async redis or threaded sync redis
        ...
        watches = [
            AlertWatch(
                ...
                alert_time=datetime.now(UTC).replace(hour=datetime.now(UTC).hour - 1 if datetime.now(UTC).hour > 0 else 23),
        ...
        quotes = {
            "AAPL": {
                "bp": 1.1, "ap": 1.2, "t": datetime.now(UTC).timestamp(),
                "symbol": "AAPL", "last_price": 1.15
            }
        }
