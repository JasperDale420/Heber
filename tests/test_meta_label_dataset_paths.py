...
def test_load_features_loads_from_configured_path(tmp_path: Path) -> None:
    ...
    ).to_parquet(dt_dir / "data.parquet", index=False)
    ...
    assert len(loaded) == 1
    assert loaded["alert_id"].tolist() == ["a1"]
...
def test_persist_features_to_gold_appends_partitions(tmp_path: Path) -> None:
    ...
    persisted = pd.read_parquet(output_file)
    assert set(persisted["alert_id"].tolist()) == {"a1", "a2"}
...
def test_persist_features_to_gold_handles_column_order_mismatch(tmp_path: Path) -> None:
    ...
    )[["alert_id", "symbol", "alert_time", "feature_x"]]
    update = pd.DataFrame(
        {
            "alert_id": ["a1"],
            "alert_time": [datetime(2026, 2, 7, 15, 0, tzinfo=UTC)],
    ...
    )[["alert_id", "alert_time", "symbol", "feature_x"]]
    ...
    assert persisted["feature_x"].tolist() == [2.0]
...
def test_persist_features_to_gold_quarantines_unreadable_existing_partition(tmp_path: Path) -> None:
    ...
    persisted = pd.read_parquet(unreadable_file)
    assert persisted["alert_id"].tolist() == ["a1"]
