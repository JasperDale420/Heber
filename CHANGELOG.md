...
- **`scripts/backfill_features.py --fill-nulls` can read the Gold dataset again**: it opened the partition tree with a raw `pyarrow.dataset()` walk, which picks up the zero-byte `data.parquet.lock` file each partition carries and fails with `Parquet file size is 0 bytes`. It now reads through `HeberReader`, which filters those.
