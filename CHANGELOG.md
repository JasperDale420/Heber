...
- **CI gate hygiene restored** (`uv.lock`, `tests/test_dead_code_cleanup.py`): refreshed vulnerable lockfile packages (`mako`, `pyarrow`, `urllib3`) to fixed releases and committed the ruff autofix that CI was applying during `pre-commit run --all-files`.
