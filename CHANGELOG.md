...
- **CI gate hygiene restored** (`uv.lock`, `.github/scripts/create-ci-stubs.sh`, `tests/test_dead_code_cleanup.py`): refreshed vulnerable lockfile packages (`mako`, `pyarrow`, `urllib3`) to fixed releases, committed the ruff autofix that CI was applying during `pre-commit run --all-files`, and aligned the CI `empire-core` stub with Heber's logger helper imports.
