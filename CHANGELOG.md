...
- **The two Gold pipelines that died every night now survive the run** (`heber/features/pipelines/market_regime_features.py`, `darkpool_features.py`): both were OOM-killed on all three attempts nightly inside the 3 GB gold-poller, burning 91 and 14 minutes per run to produce nothing. `market_regime` read every equity ticker's bars over a ~127-day window (~6.4M rows) and discarded all but ~160k; it now reads one day at a time and reduces to daily closes before moving on. `darkpool` was the only Gold pipeline with no column projection, batching or chunking — it read whole rows for the full universe over ~37 days, when only `ts_event`, `underlying`, `notional` and `premium` are used. `market_regime_features` has not been written since 2026-06-09 as a result.
