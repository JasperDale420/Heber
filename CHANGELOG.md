...
- **An unreadable feature dataset now fails the training job instead of looking empty** (`heber/ml/datasets.py`): `_load_features` caught every exception and returned an empty DataFrame, which is what let a total read failure pass as "no alerts in range". It now re-raises. (`_load_outcomes` still swallows errors the same way and is worth the same treatment.)
