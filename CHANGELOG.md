...
- **Opt-in durable JetStream ingestion** (`heber/writer/{jetstream_consumer,backfill_ack}.py`): live and backfill lanes can independently consume bounded JetStream work queues with explicit ACKs while reusing the existing normalization, dedupe, Bronze, Silver, and filesystem DLQ pipeline. The backfill consumer can be switched first while live remains on Redis. ACKs occur only after both layers and a commit receipt are fsynced. Backfill chunks additionally publish an exact, idempotent Redis control-plane proof after all expected event and payload digests match; proof-store failures, malformed lineage, and filesystem DLQ failures remain unacknowledged for redelivery. Readiness records identify the bound transport, lane, stream, and durable consumer so Gateway cannot publish into an unconsumed lane. Pending recovery handles a complete 5,000-record proof chunk, and finalized chunks remain ACK-eligible across transient Redis or JetStream acknowledgement failures. Redis remains the default transport until cutover fault tests pass.

- **A read-only audit of stranded stream deliveries** (`scripts/debug/audit_stranded_pel.py`, findings in `docs/operations/audit-2026-07-29-stranded-pel.md`): reports, per consumer group, how many delivered-but-unacknowledged messages exist, how many have aged past the stream's retention window and can therefore never be re-read, and which time windows they span. The 2026-07-29 capture found **34,745 permanently unrecoverable deliveries** on the live stream plus **5,000 still-recoverable ones** stranded on the backfill stream by a consumer that died six days earlier. The script deliberately does not estimate how many rows were lost — a trimmed entry carries no feed or payload, so that question needs a provider re-pull and diff, and any other number would be a guess.
...
- **Fails-to-deliver features can now actually be produced** (`heber/features/pipelines/market_intel_features.py`): the Gold poller runs with a one-day lookback, and the FTD dataset both read and filtered to that window — but the SEC publishes fails-to-deliver data roughly 15–30 days after the settlement date it describes, so that window can never contain an FTD record. The dataset produced zero rows every night by construction, and had done since it was written. FTD now reads and filters over a 45-day window; the other datasets in the pipeline keep the short one, since widening theirs would multiply read cost for nothing. All reads in this pipeline also now prune non-matching date partitions instead of opening every partition directory to test each row's timestamp — Silver holds about a thousand day-partitions of FTD data, so the unpruned read was opening tens of thousands of files to discard almost all of them.

- **One alert now produces one watch, however many times it is delivered** (`heber/watch/manager.py`, `heber/watch/models.py`): `create_watch` generated a fresh watch id on every call, so the same alert processed twice created two independent watches. Both then polled the same contract on their own schedule and each wrote its own Gold label rows — duplication that lands directly in ML training data, where the compactor's event-id deduplication cannot see it. An alert now atomically claims its watch (`SET NX`), and a repeat delivery returns the existing watch instead of creating another. Nothing triggered this while the consumer only ever read new messages; it becomes reachable as soon as anything redelivers — pending-message recovery, a restart replaying unacknowledged work, or any at-least-once transport.

- **Redis acknowledgements now follow an intact-filesystem durability boundary** (`heber/writer/{consumer,bronze,silver,utils}.py`): completed Bronze and Silver files and their parent directories are fsynced around the atomic rename, and a compact fsynced batch receipt is written before dedupe registration or acknowledgement. Sub-threshold batches remain unacknowledged until the existing size, 30-second, or 50,000-message bound forces a commit. If either layer or the receipt fails, the Redis messages remain pending for redelivery.

- **Filesystem DLQ captures survive host power loss before poison-message ACK** (`heber/writer/dlq_fallback.py`): the atomic fallback write now fsyncs the completed file, its partition directory, and the fallback root before either Redis or JetStream may acknowledge the failed source message.

- **Background services no longer uninstall your development tools** (`scripts/run_native_heber_service.sh`): the launchd service runner called `uv sync`, which removes every package outside the project's default dependency set — and five of those jobs run every few minutes against the same virtualenv you work in. The practical effect was `pytest`, `mypy`, and `ruff` disappearing mid-session, repeatedly, with no indication why. The runner now uses `uv sync --inexact`, which still installs everything it needs but leaves anything else alone.

- **Three defects found by adversarial review of the durability work** (`heber/writer/consumer.py`, `heber/writer/dlq_reprocessor.py`): recovery could stall behind messages the consumer was already holding, because the pending scan is oldest-first with no cursor and a page made entirely of held ids produced no candidates, which read as "nothing left to recover" — it now pages past them. The backpressure branch retried a failing flush with no pause, turning a stuck disk into a hot loop; it now backs off. And the DLQ reprocessor, which flushes the writers itself rather than going through the consumer's commit, never registered its reprocessed events in the deduplication store, so a later replay could write them again — it now registers them after its flush.

- **The consumer now flushes on every shutdown path** (`heber/writer/consumer.py`, `docker-compose.yml`): the final flush sat after the main loop rather than in a `finally`, so two exits skipped it entirely — cancellation, and a fatal Redis error — and whatever was buffered died with the process. It now always runs, with each layer isolated so one broken writer cannot skip the other, and a failure there is logged rather than replacing the exception that caused the shutdown. `stop()` no longer closes the Redis connection, which previously made a final acknowledgement impossible; the connection is closed last, after the flush and commit. Both consumer containers now get a 60-second `stop_grace_period`, because Docker's 10-second default was killing them mid-flush.

- **Recovered messages now commit through the same durable path as live ones** (`heber/writer/consumer.py`): the pending-message recovery drain acknowledged claimed messages directly, which after the deferred-acknowledgement change would have left their events registered as "seen" but never written to the deduplication store, and never released. Recovery now holds and commits exactly like the main loop, skips any message the consumer is already holding (reclaiming one would write the same event twice), and reports only the messages it actually acknowledged rather than everything it claimed.

- **Events are no longer acknowledged before they have been written** (`heber/writer/consumer.py`): the consumer told Redis it had handled a batch as soon as the flush call returned without error — but that flush only writes partitions past a size or elapsed-time threshold, so on a quiet iteration nothing reached storage and the batch was acknowledged anyway. Because an acknowledged message is never redelivered, a container kill lost up to a flush interval of events, silently. The consumer now holds message acknowledgements until a flush confirms no buffered event remains, and forces a full flush if the held set gets too large (`HEBER_WRITER_MAX_UNACKED_MESSAGES`, default 50,000) or too old (`HEBER_WRITER_MAX_UNACKED_SECONDS`, default 30s) so the pending list cannot drift toward the stream's retention limit. If flushing stops working entirely the consumer now stops reading rather than growing an unbounded backlog. Deduplication was also recording each event as seen *before* the write, so a redelivery of data that had never been written was discarded as a duplicate — that registration now waits for the same durable commit. The trade is deliberate and worth stating plainly: **fewer lost events, occasionally duplicated ones.** Bronze is append-only and the compactor removes duplicates by event id, so a duplicate is recoverable where a loss is not. New metrics: `heber_consumer_pending_ack_messages`, `heber_consumer_pending_ack_age_seconds`, `heber_consumer_forced_flush_total`.

- **The writers now report honestly whether everything reached storage** (`heber/writer/bronze.py`, `heber/writer/silver.py`, `heber/writer/consumer.py`): `_flush_layers()` previously returned success whenever no exception was raised, but the flush it calls only writes partitions past a size or elapsed-time threshold — so on a quiet iteration it wrote nothing, reported success, and the consumer acknowledged the batch with the events still in memory. It now returns true only when no buffered event remains. Both writers gained `has_buffered()`, and `flush_if_needed()` now reports whether it wrote anything.

- **Silver rows rejected by schema validation are now recorded instead of vanishing** (`heber/writer/utils.py`): when a partition failed Arrow type coercion, the writer retried row-by-row and discarded whatever still failed — the all-rows-invalid case logged a single line and reported success, and the partial case logged only the survivors, at debug level. Either way the caller treated the flush as successful, dropped the buffer, and acknowledged the message, so the rejected rows existed nowhere afterwards. Rejected rows are now written to the DLQ fallback directory with their partition, reason, and event ids, counted in the new `heber_writer_silver_rows_rejected_total` metric, and logged at ERROR. Bronze still holds the raw envelopes, so a schema drift is now both visible and replayable. A partition whose rows are all invalid still completes rather than raising — raising would leave it buffered forever and stall the consumer.

- **A single bad partition no longer discards every partition behind it at shutdown** (`heber/writer/bronze.py`, `heber/writer/silver.py`): the unconditional `flush()` — the path that runs when a writer is shutting down or must get everything on disk before acknowledging — walked partitions serially and propagated the first error, so every partition after the failing one was never written and its buffered records were dropped with the process. Both writers now use the same concurrent helper the threshold-based flush already used: every partition is attempted, one is dropped only once its write succeeds, a failed partition keeps its records, and the first error is still raised so the caller does not acknowledge.

- **`heber-watch` now writes its durable DLQ audit files instead of losing them** (`docker-compose.yml`): it was the only Heber service that never set `HEBER_DATA_ROOT=/data`, so every path derived from `data_root` fell back to the host path `/Volumes/heber/data`, which does not exist inside the container. When a Redis DLQ write failed — which happens on every Redis restart — the last-resort on-disk fallback failed too (`PermissionError: '/Volumes'`), leaving the failed alert with no audit record anywhere. `HEBER_GOLD_PATH=/data/gold` had masked the problem for Gold writes only.

- **The nightly lakehouse backup now completes instead of running for days** (`scripts/backup_lakehouse.sh`): it mirrored Bronze file-by-file — millions of ~2 KB objects copied one at a time onto the removable exFAT drive — which on that medium never finished (two runs were caught stalled at 21 h and 23 h, still on Bronze, so the lakehouse had *zero* verified backup). Bronze is now bundled into a single `.tar` per `provider/feed/dt` day-partition, and only new or backfilled days are re-archived (detected by directory mtime, so a nightly run tars just the day's new partitions and dir-checks the rest in minutes). Silver/Gold stay as Parquet mirrors, and the catalog Postgres is now captured with a consistent `pg_dump` via the running container instead of an unrestorable copy of its live data directory. Operational note: the launchd job (`~/Library/LaunchAgents/com.empire.heber.backup.plist`) must invoke the script through `/bin/bash` explicitly, and `/bin/bash` needs macOS **Full Disk Access**, or the job cannot read the external volumes (silent `Operation not permitted`). The plist must also **not** set `ProcessType: Background` / `LowPriorityIO` — macOS throttles a background-QoS job's disk I/O to a few KB/s while the volumes are busy, which stretched the first full seed from hours to weeks; it now runs at `ProcessType: Standard`.
