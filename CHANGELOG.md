...
- **A rare disk hiccup could leave a corrupted, empty options-data file that looked fine** (`heber/ml/datasets.py`, `scripts/normalize_gold_expiry.py`): if the machine lost power or a process was killed at exactly the wrong moment during a write, the file left behind could appear present but actually be empty or broken — the storage volume this data lives on doesn't have the safety net most drives do. Writes to this dataset are now more careful about confirming the data really reached disk before considering the write finished. Several related rough edges found while fixing this: a failed write for one day of data no longer throws away progress already made on other days in the same batch, and the caller is now told when that happened instead of the batch quietly looking like a full success; a failed verification during the cleanup script's rewrite now keeps the broken file around to diagnose instead of deleting the only evidence of what went wrong, and that kept evidence is itself now written to disk properly instead of risking the exact same "looks present but isn't really there" problem this whole fix exists to close.
  ...
  Two deeper gaps surfaced during review and are intentionally not part of this fix, since closing either properly means changing code shared by several other parts of the system rather than a targeted change here — both are tracked separately: on this Mac, even a "successful" disk flush isn't always as durable as it would be on Linux, for storage attached the way this volume is ([#55](https://github.com/JasperDale420/Heber/issues/55)); and separately, a live alert whose feature data fails to save today still gets marked as fully handled rather than flagged for a retry, a pre-existing tradeoff (not something this fix changed) that trades a small amount of possible data loss for never blocking on a save failure ([#58](https://github.com/JasperDale420/Heber/issues/58)).
