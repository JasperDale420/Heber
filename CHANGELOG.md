...
- **`ticker_base_rates` no longer trains a live ML feature on mislabeled outcomes** (`heber/features/pipelines/ticker_base_rates.py`): 171,426 of 191,262 rows in `labels_alert_barriers` carry `quality_flags=["label_window_truncated"]` — their `hit_tp_first` reflects the collapsed ~3-5h barrier-check window from the `MarketCalendar.add_trading_hours` session-boundary bug, not the alert's real SWING/LEAP horizon. This dataset's win-rate feeds `EQUITY_TICKER_RATES_FEATURES`, a live model input Orion asof-joins onto every alert it scores or trains on, so the contamination reached inference, not just reporting. `compute_ticker_base_rates` now drops flagged rows via `heber.ml.datasets.quality_flag_series` before computing rolling stats — both as priors in another alert's window and as a row's own output — matching the exclusion `MetaLabelDatasetBuilder` already applies to the same flag as a training target. Coverage shrinks accordingly (clean rows span 101 of 124 days, median 13/day, almost entirely INTRADAY) until the label backlog is regenerated or enough clean data accumulates post-fix; the existing `no_data` status already handles a window with nothing left in it.
