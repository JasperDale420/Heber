...
- **The statistical audit now reads the whole trading day instead of a single instant** (`heber/health_monitor/checks/statistical.py`): it asked for a window running from a date to the *same* date, which both resolve to midnight, so only rows landing exactly on that instant could match. Every intraday feed — `bars` included — came back empty and was skipped as "no data", which is why no `statistical_*` result has ever been recorded. The audit now covers the full day, and skips partitions outside it rather than scanning a feed's entire history to profile one day.
