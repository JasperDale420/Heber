...
- **Model training reads the full `meta_label_features` dataset again — 117,824 rows instead of zero**: a whole-dataset read was failing outright (`Unsupported cast from double to null`), and `MetaLabelDatasetBuilder` turned that failure into an empty DataFrame, so training looked like it had simply found no alerts. Three separate causes, all now fixed: 19 columns had drifted to conflicting physical types across partitions; the reader was loading 10,396 quarantined files that were never meant to be read; and the loader was masking the error. Verified end to end through the production reader.

- **Quarantined rows no longer leak back into training reads** (`heber/reader/core.py`): rows whose Greek enrichment is entirely null are diverted to `_quarantine/`, on the documented assumption that readers only load `dt=` partitions. pyarrow's dataset walk is recursive, so it picked them up anyway — 10,396 files in `meta_label_features` alone, which both undid the quarantine and broke the dataset-wide read, since parked files keep whatever schema they had when they were set aside. The reader now skips any `_`-prefixed directory beneath a dataset root.

- **The outcomes half of the training set loads again — 194,630 rows instead of nothing**: with features readable, the outcomes side turned out to be failing too. A single zero-byte parquet file in `labels_alert_barriers` (`dt=2026-06-09`) failed the scan for the whole dataset, and `_load_outcomes` turned that into an empty frame, so every outcome row was invisible.

- **A zero-byte parquet file no longer takes down the dataset that contains it** (`heber/reader/core.py`): the lakehouse volume is exFAT, where a rename without an explicit flush can publish an empty file, and one such file made every row in `labels_alert_barriers` unreadable. Empty files are now skipped like the `*.tmp` partial writes they resemble, and named in a warning. A `strict=True` read refuses the dataset outright, because the rows the empty file should have held are lost and uncountable — a training caller asking for the complete dataset must not silently receive a partial one.

- **Gold feature and label writes are flushed before they are published** (`heber/ml/datasets.py`, `heber/watch/writer.py`): both wrote a temp file and renamed it without an `fsync`, so on this volume the rename could land ahead of the data and publish exactly the kind of zero-byte file described above. Both now fsync the file and its directory around the rename.

- **An unreadable feature or outcome dataset now fails the training job instead of looking empty** (`heber/ml/datasets.py`): `_load_features` and `_load_outcomes` both caught every exception and returned an empty DataFrame, which is what let total read failures pass as "nothing in range". Both now re-raise, via a new `strict=True` mode on `HeberReader.read_parquet_dataset`. A missing path still returns empty — that is a genuine absence rather than a failure.

- **Gold `meta_label_features` partitions no longer drift in physical type**: `expiry` existed as `date32`, `int64` and `string` across partitions, `alert_time` at two different timestamp resolutions, and 17 more columns as Arrow `null` wherever a day's rows happened to be entirely null. All 120 partitions were normalized to the declared types in `heber/schemas/gold.py`, and the writer now pins those types instead of trusting pandas inference. A column written with an inferred `null` type is logged as a warning naming the column, since that is how the drift started.

- **Three Gold `meta_label_features` partitions are readable again — 1,481 alert rows restored to model training** (`dt=2026-04-14`, `dt=2026-05-06`, `dt=2026-06-17`): each held a single row whose option `expiry` had been written as the integer `20260819`-style `YYYYMMDD` into a `date32` column, which pyarrow reads as a raw day count — year 57442. That one value made `pd.read_parquet` raise for the *whole* partition, so `MetaLabelDatasetBuilder` and `scripts/backfill_features.py` skipped all three silently. The bad values were corrected against the expiry embedded in each row's OCC symbol; row counts and `alert_id` sets are unchanged.

- **An out-of-range `expiry` can no longer reach the Gold layer** (`heber/ml/datasets.py`): `persist_features_to_gold` — the single write path for `meta_label_features`, shared by the live watch consumer and the enrichment backfill scanner — now normalizes `expiry` through a strict `normalize_expiry()` and pins the column to Arrow `date32` instead of trusting pandas type inference. Values it cannot parse unambiguously (trailing garbage, partial dates like `202609`, non-integral floats) become null and are logged as an error rather than being guessed at, and legacy string/int partitions are converted to `date32` the next time they are appended to. Inference was the actual failure mechanism: a column of all-null values inferred as Arrow `null`, and a mixed column took whichever type it saw first.
...
- **`heber/schemas/gold.py` — declared Arrow types for Gold datasets**: `META_LABEL_FEATURES_TYPES` pins the physical type of each known `meta_label_features` column, following the existing `SILVER_SCHEMAS` pattern. Declared columns are coerced on write; undeclared columns pass through untouched, so adding a feature cannot silently drop it.

- **`scripts/normalize_gold_schema.py` — retypes existing Gold partitions to the declared types**: converts all-null columns to typed nulls, parses `YYYYMMDD` integer/string expiries into real dates, and unifies timestamp resolutions. It rejects any rewrite that changed a row count, a column set, or a single value — typing only. The dry run performs the full conversion and validation for every partition and then reports whether the whole dataset reads, so the check is the same one the write path applies.

- **`scripts/repair_gold_expiry.py` — repairs unreadable `expiry` values in Gold feature partitions**: finds `date32` values outside a plausible range (the `YYYYMMDD`-as-day-count bug) and corrects each one only when re-reading it as `YYYYMMDD` agrees with the expiry embedded in that row's OCC symbol. A partition where any row fails that cross-check is left untouched, because a partial repair is worse than a known-bad file. Dry-run by default; `--write` copies each original to `~/heber-repair-backups/<run>/` with a SHA-256 manifest first, takes the same partition lock the writer uses, fsyncs before publishing (the lakehouse volume is exFAT — a rename without an explicit flush has published zero-byte files before), then re-reads the result to confirm row count, `alert_id` set, and every non-`expiry` column are unchanged.
