...
- **A consumer that is alive but not ingesting is now alerted on** (`heber/ops/stack_check.py`): new `stack_ingest` check, scraping the consumer's `heber_consumer_last_xread_success_unixtime` from its loopback metrics port and paging when no successful stream read has happened in 5 minutes. The container healthcheck cannot see this — it reads the run-loop heartbeat, which stays fresh while the loop spins on a dead upstream. The bound clears a routine ~77s Redis AOF reload.
