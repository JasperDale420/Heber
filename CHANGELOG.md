...
- **Massive REST retry/pagination-guard logic is unified across the daily corporate-actions downloader and the resumable REST backlog sweeper** (`heber/backfill/massive/http.py` new, `heber/backfill/massive/corporate_actions.py`, `heber/backfill/massive/rest_backlog.py`): `MassiveCorporateActionsDownloader` and `MassiveRestBacklogSweeper` each carried their own byte-for-byte copy of the Bearer-auth GET, the exponential-backoff retry loop, the gzip-JSON page writer, and the `next_url` host-pinning guard. The two copies had already drifted apart in retail (the sleep cap differed, 60s vs 90s) with nothing to keep them from drifting further. Both classes now call the one shared implementation in `heber/backfill/massive/http.py`, each still passing its own sleep cap so behavior is unchanged. No test or call-site changes required — both classes' public APIs are unchanged. The three sibling `heber/backfill/massive/*_sweep.py` one-off scripts were deliberately left alone: they are intentionally stdlib-only (no `structlog`, no dependency-injected `http_get`) so they can run under a bare `python3` with no environment sync, which the shared module (via `corporate_actions.py`/`rest_backlog.py`) does not preserve.

- **Soda data-quality scanning is replaced by native Silver invariant checks** (`heber/quality/silver_invariants.py`, `heber/ops/daily_health.py`): the Soda integration had not run at all for some time and nothing noticed. `soda-core` 3.3.20 imports `distutils`, removed from Python 3.12, so `from soda.scan import Scan` raised on every interpreter Heber supports; the daily health check caught that and reported a soft warning, so a third of the daily report was quietly returning nothing. The latest 3.x DuckDB adapter still has the same defect, and Soda 4 is a different product with a different check language, so the integration is gone rather than upgraded.
  Most of what those checks asserted is already covered by `heber.health_monitor` — row counts and freshness (partition, liveness), per-column null rates (statistical), schema drift (schema) and volume trends (volume). What was genuinely missing is whether the values themselves make sense, and that is what the replacement covers: negative prices, sizes and volumes; bars whose high is below their low or below open/close; crossed quotes; duplicate `event_id`s; instrument types outside the contract; and the zero-leakage invariant (`ts_available >= ts_event`, and never in the future) on Silver, which `ml_readiness` only audits on Gold.
  It also runs against the report date's partitions rather than every Parquet file a feed has ever written, which is what made the old scan unable to finish. On its first real run it found 9 duplicate-`event_id` rows in crypto trades out of 65,133 checked.
  One retired check was deliberately not carried over: the allow-list for `flow_alerts.alert_type` named five values the provider has never sent, so every row would have failed it. That vocabulary is vendor-controlled and open-ended, so pinning it produces false alarms rather than findings.
  The replacement reports what it could *not* check, not just what failed. An invariant whose columns are absent, a predicate that raises, or a partition that cannot be read all make the check non-passing and are named in the report, with a `scan_complete` flag alongside. That matters because the retired checks also named `bid_price`/`ask_price` for a feed whose columns are `bid_px`/`ask_px` — every quote check matched nothing and looked green. A test now asserts every invariant's columns against `SILVER_SCHEMAS`, so a rename breaks a test instead of silently disabling a check. Values that are present but unparsable count as violations rather than being coerced to null and skipped, and `event_id` uniqueness is tallied across a feed's whole day rather than within one `instrument_type` partition.
  Per-column completeness contracts are carried over rather than delegated: the retired checks held bars `open`/`high`/`low`/`close` and trades `price` to under 1% missing, while health_monitor profiles null rates against a single 5% default, so leaning on it alone would have quietly relaxed those columns. Fill rates are tallied across a feed's whole day, not per partition.
  Crossed quotes keep both halves of the contract the retired checks gave them: tolerated below 1% of a feed's rows, and a *warning* above it rather than a critical failure of the whole daily report. Partitions are read through the canonical `_collect_parquet_files`, so `._*` AppleDouble sidecars and `part-*.parquet.tmp` partial writes stay ordinary noise instead of turning a healthy partition into a failed check. Schema drift is deliberately *not* re-checked here: detecting it is not what a value-level scan is for, and a partition-scoped copy of that rule would be weaker than, and disagree with, the reader's own cross-fragment contract.

  **Found while doing this, not fixed here:** `heber/health_monitor/checks/schema.py` — the check that nominally owns schema drift — is itself inert for the same class of reason Soda was. It looks for `feed=<feed>/dt=<date>`, but the writer's layout is `feed=<feed>/instrument_type=<type>/dt=<date>`, so the directory never exists and every feed is skipped: measured today, it would examine 0 of 4 feeds. Silver schema drift is therefore currently unmonitored. Tracked separately rather than widened into this change.

  Note on status: a warning still exits `heber health-daily` non-zero, as every non-`ok` status already did. The warn/fail split changes what the report *says*, not whether a scheduled run is considered to have failed.

  **Known gap, deliberately left open:** `quotes` is the only Silver feed with no liveness rule, so it has no freshness guard. The retired checks nominally held it to a 30-minute SLA, but since they never executed, that SLA has never actually been enforced — this changes nothing in practice. A single liveness rule cannot cover it correctly: `crypto_quotes` normalizes into the same `quotes` feed, so one crypto quote could mask an equity outage, and the liveness runner is gated by the US equity calendar, which would skip crypto entirely on weekends and holidays. Closing it properly means splitting quote liveness by instrument type and session, which is its own change.
  `generate_daily_health_report`'s `skip_soda` argument is now `skip_invariants`. Removing the dependency also drops `sqlparse` from the lockfile entirely, along with 18 other transitive packages that only Soda needed.

### Removed

- **Three unused Silver Pydantic models are gone from `heber/models/silver.py`**: `IncomeStatementRecord`, `BalanceSheetRecord`, and `CashFlowRecord` were added alongside the `income_statement`/`balance_sheet`/`cash_flow` feeds (PR #32) but, unlike every one of the other ~50 documented Silver record models in that file, were never added to `heber/models/__init__.py`'s exports, never imported by any writer/reader/test, and never referenced outside their own definitions. The actual Silver schemas for those three feeds are the `pa.schema(...)` entries already registered in `heber/schemas/silver.py`'s `SILVER_SCHEMAS`, which is what the writer validates against — these Pydantic classes were pure dead documentation that had drifted out of the loop. No behavior change: nothing constructed or imported them.
- **`MissingInstrumentIdentifierError` is gone from `heber/writer/key_normalization.py`**: this exception subclass was defined ("Raised when a feed cannot produce a required instrument identifier") but never raised, caught, or imported anywhere in this repo or any sibling repo. The sibling `InvalidInstrumentKeyError` is the one actually used. Purely dead code with no test coverage.
- **`check_dlq_size` is gone from `heber/writer/dlq_reprocessor.py`**: this Redis-`XLEN`-based DLQ size check was left behind by the earlier migration to a durable-file-count-based check (`_collect_dlq_size_check` in `heber/ops/dataflow_health.py`, see the "DLQ size alert now reflects real failures, not Redis bookkeeping" entry below) — the module still carried a stale `# importable by dataflow_health` comment, but `dataflow_health.py` never imported it, no other module or script called it, and it had no test coverage. Purely dead code; the CLI (`--analyze`/`--reprocess`/`--purge`) is unaffected.
...
- **The dead-man can run on GitHub instead of a third-party account** (`.github/workflows/heartbeat-deadman.yaml`, `HEBER_ALERT_HEARTBEAT_GITHUB_REPO`): the heartbeat needed somewhere off-machine to watch from, and the only option was signing up for healthchecks.io. `alert-check` can now stamp a variable on the private repo each cycle, and a scheduled workflow on GitHub's runners reads it — off-machine, using the `gh` login that already exists, publishing nothing. It alerts to Discord when the stamp goes stale (the alarm is gone) and, separately, when the stamp says `fail` (the alarm is alive but its run crashed or a Discord send did not land). The healthchecks.io path still works and both can run together.
  Scheduled workflows only fire from the default branch, so this file has to sit on `master` to run at all; it depends on nothing else in the repo. Staleness is judged at 30 minutes — six alert-check cycles — because GitHub delays cron under load and a tighter bound would invent outages.
- **A pause and OOM recorder for the Docker stack** (`launchd/com.empire.heber.pause-events.plist`, installable via `scripts/install_native_launchd.sh pause-events`): on 2026-08-12 `heber-watch` sat *paused* for two hours while the watchdog retried `compose up` against it every 2 minutes and failed with "cannot start a paused container"; nothing on the host recorded what paused it, and nothing in the Empire tree issues `docker pause`. The watchdog now unpauses within 120s, which erases the evidence before anyone can look. A long-lived `docker events` stream now records every pause, unpause and OOM kill to `logs/native/docker-pause-events.log`. `oom` is included because a container with no cgroup limit is killed at the VM level with no `OOMKilled` flag and no exit code — the daemon event is the only durable record.
...
- **A completed watch can no longer lose its Gold label to a malformed Redis key** (`heber/watch/manager.py`): completing a watch and durably staging its outcome were done in one Redis `MULTI`/`EXEC`, which is not a transaction in the rollback sense — Redis queues the commands and then runs all of them, so one command failing at execution time (a `WRONGTYPE`, if something out of band left a string where `heber:watch:pending_outcomes` expects a set) leaves the commands around it applied. That half-applied state is a permanent loss: the watch is gone from the active set, so no checker pass ever revisits it, while its outcome never reached the pending index, so no retry pass ever finds it. Both halves now run in a single Lua script, which validates every key's type before its first write, so a bad key aborts with nothing mutated and the watch stays active and retryable. Because Lua gives isolation rather than rollback — a later command can still fail on its own, and Redis will not undo the ones before it — the script's writes are also ordered so that every partial application is recoverable: the outcome is staged and indexed first, and only then is the watch retired from the active set. Failing early leaves the watch fully active for the next check; failing late leaves the outcome already indexed for the retry pass. The one overlapping state (staged but still active) costs at most one redundant re-check, which overwrites the same deterministically-named Gold file rather than duplicating a row. Surfaced by two rounds of adversarial review (Codex, gpt-5.6-terra) of the merged durability work; the Lua is now exercised against a real Redis in CI, not only against test doubles.

  **Known limit, unchanged by this fix:** clearing a staged outcome still matches on `watch_id` rather than on the payload, which assumes a single writer. That holds in the current deployment — `heber-watch` runs as one instance and stages, writes, and clears sequentially in one loop — but running two watch services would let one stage a newer outcome for a watch between another's read and clear, discarding it. Closing that needs a versioned payload and a compare-and-delete; it is noted at `clear_pending_outcome` so anyone scaling the service out finds it first.

- **Watches that expired via the poller's cleanup pass silently lost their label** (`heber/watch/manager.py`, `heber/watch/poller.py`): `SnapshotPoller.run()` called `WatchManager.cleanup_expired()` on its own timer, which completed expired watches directly from their mutable `current_return`/`snapshot_count` fields — bypassing `BarrierChecker` entirely. That meant no Gold label row was ever written for a watch finalized this way, and (before the fix above) the return used wasn't windowed to `[alert_time, window_end]` either. It also raced `BarrierChecker.check_all()`, which runs every 60s and already visits every active watch — including any whose window has passed, which it force-expires with a real, windowed `WatchOutcome`. On a service restart with a backlog of already-expired watches, the poller's sweep could reach one before the checker loop's first tick, permanently dropping that watch's label. `cleanup_expired`/`cleanup_expired_async`/`get_expired_watches` are removed; `BarrierChecker.check_all()` (driven by the existing check-and-write loop) is now the sole place watch completion happens.
- **A watch label's `ts_available` could be backdated to a stale business timestamp** (`heber/watch/checker.py`): `outcome_to_label_row` stamped `ts_available` from `outcome.outcome_time`, which for an `EXPIRED` outcome is `window_end` — potentially hours or days in the past for a watch recovered from a service-restart backlog (see the fix above, which made `BarrierChecker` the guaranteed sole path for those). A value only becomes queryable once it's actually written; backdating `ts_available` to when the window merely *closed* would let a point-in-time read between `window_end` and the actual write see a row that didn't exist yet, violating the zero-leakage contract. `heber/watch/backfill_scanner.py` already encodes this exact principle for a different write path ("a value fetched now only becomes queryable now"); `ts_available` is now stamped at write time here too, clamped to `outcome.alert_time` so a future-dated or clock-skewed alert can't produce `ts_available < ts_event` (mirroring the same `max(now, event)` guard `backfill_scanner.py` already applies). Found via two rounds of adversarial review (Codex, gpt-5.6-terra) of the fix above.
- **`ticker_base_rates` no longer computes a live ML feature from mislabeled outcomes going forward** (`heber/features/pipelines/ticker_base_rates.py`): 171,426 of 191,262 rows in `labels_alert_barriers` carry `quality_flags=["label_window_truncated"]` — their `hit_tp_first` reflects the collapsed ~3-5h barrier-check window from the `MarketCalendar.add_trading_hours` session-boundary bug, not the alert's real SWING/LEAP horizon. This dataset's win-rate feeds `EQUITY_TICKER_RATES_FEATURES`, a live model input Orion asof-joins onto every alert it scores or trains on. `compute_ticker_base_rates` now drops flagged rows via `heber.ml.datasets.quality_flag_series` before computing rolling stats — both as priors in another alert's window and as a row's own output — matching the exclusion `MetaLabelDatasetBuilder` already applies to the same flag as a training target. Coverage shrinks accordingly (clean rows span 101 of 124 days, median 13/day, almost entirely INTRADAY) until the label backlog is regenerated or enough clean data accumulates post-fix; the existing `no_data` status already handles a window with nothing left in it.

  **Known gap, not yet remediated:** this only changes what future pipeline runs compute. `gold/dataset=ticker_base_rates` already has 84 part-files across 42+ `dt=` partitions (since at least 2026-04-15) written by the old, unfiltered code, and `write_gold` appends new parts rather than replacing existing ones — reads concatenate every part in a partition with no dedup by `(instrument_key, ts_event)`. Orion is reading contaminated rows from that dataset right now, and re-running this pipeline over the historical range would add clean rows *alongside* the contaminated ones rather than superseding them, producing duplicate-keyed rows with conflicting win-rates. A separate remediation pass (rebuild-and-replace the affected partitions, or a new dataset version Orion is repointed to) is required before this fix has any effect on what's currently served; flagged for the user to decide before executing, since it touches a shared, non-append-safe production dataset.

- **`scripts/rebuild_ticker_base_rates.py` closes the gap above** — a one-shot migration that fully replaces the existing contaminated output rather than appending beside it. Measured against production: of the 84 old files / 56,053 rows (10,822 distinct ticker-days), **only 749 rows (273 ticker-days, 49 tickers) survive** under the corrected pipeline over the same range — 98.7% of what Orion is currently reading doesn't correspond to a real signal at all, and the 144 ticker-days that do overlap are still wrong by a median 11.8 win-rate points (max 88.7) with a median 1,076 phantom prior alerts each.

  Orion does **not** read this dataset through `HeberReader` — its own reader (`orion/clients/heber_reader.py`) globs every `.parquet` under `dataset=ticker_base_rates` directly, with no version pinning and no `_`-prefixed-directory exclusion at all. That rules out writing a clean `version=v2` (Orion would union it with the still-present v1) and rules out backing old data up anywhere inside `dataset=ticker_base_rates/`, `_`-prefixed or not. The backup goes to `gold/_migrations/ticker_base_rates_rebuild/<run>/`, a sibling of the dataset entirely outside it — confirmed against every other path in this repo that walks the Gold root (`scripts/compact_gold.py` filters to `dataset=*` only; `scripts/data_health_audit.py` does not filter, but is a manual, non-automated diagnostic tool with no Orion or production dependency).

  New data is written into the live `dt=` partitions and verified on disk, per partition, **before** any old fragment is touched — old data is only backed out once every new file's row count is confirmed to match. At no point does the dataset appear empty or partial to a concurrent reader, even under a crash mid-run; the worst case is old and new data briefly coexisting, no different in character from the multi-fragment partitions this dataset already has. `--expect-old-rows`/`--expect-new-rows` are validated in a read-only planning pass *before* the destructive call is ever made, not just checked after. The scheduled nightly pipeline does not participate in this script's lock — add `ticker_base_rates` to `HEBER_GOLD_POLLER_DISABLED_PIPELINES` before running `--apply` — but the write-new-first ordering bounds a missed disable to one harmless coexisting file, never lost or corrupted data. Dry-run confirmed against production twice (once before, once after an adversarial-review redesign): identical numbers both times, and the dataset verified untouched afterward.

- **Alert labels resolved against a truncated observation window are now identified and kept out of training** (`heber/ml/datasets.py`, `heber/gold/partition_lock.py`, `scripts/flag_truncated_label_windows.py`, `scripts/compact_gold.py`): `MarketCalendar.add_trading_hours` reset its cursor to the current session's open on every boundary crossing, so a 720 trading-hour LEAP window landed at open + 5h on the alert day and a 120-hour SWING window at open + 3h — fixed in `b71d63e2`, but every label already written was resolved against the short window. **174,787 of 194,630 rows** in `gold/dataset=labels_alert_barriers` are affected, spanning the dataset's entire range (**2026-01-28 → 2026-08-14**): every SWING row (95,953), every LEAP row (77,895), and 939 INTRADAY rows whose window closed *before* the alert (a separate timezone relabel in the same function). Their `expired` labels mean "no barrier touched in a few hours", not "in 5 or 30 days".

  These labels cannot be regenerated. The only per-contract point-in-time price history in the lakehouse is `option_chain_snapshot` (SPY/QQQ/IWM only, from 2026-03-10) and ~34 days of option quotes in Bronze and Silver (2026-06-16 → 2026-08-04, with a week missing) — just **265 of the 174,787 rows** have their whole corrected window inside that coverage, and **72,500** have corrected windows that end in the future and cannot be labelled by anyone yet.

  So they are marked instead. A new `label_window_truncated` quality flag records the verdict, and `MetaLabelDatasetBuilder` now excludes those rows by default (`DatasetConfig.include_truncated_label_windows` opts back in). The check is calendar-free: `add_trading_hours` only ever advances its cursor, skipping non-trading time rather than consuming it, so a correct window always spans at least its nominal count of *wall-clock* hours and anything shorter cannot have come from an intact calendar. That predicate reproduces a full per-row recomputation against the fixed calendar on all 194,630 rows exactly. Outcomes that carry no `horizon` or `window_duration_hours` are now rejected rather than trained on unchecked.

  `scripts/flag_truncated_label_windows.py` back-fills the flag onto the existing rows — dry run by default, `--expect-rows`/`--expect-flagged` to pin the cohort so a partial mount fails instead of reporting a clean pass, and a manifest written beside the data. It only ever adds to `quality_flags`; no other column is touched and no row is moved, added, or dropped. The migration has been run: 194,630 rows scanned, **174,787 flagged** across 123 of 125 partitions, no fragment unreadable. Verified afterwards — row count unchanged at 194,630, the `watch_id` set identical, every non-`quality_flags` column byte-identical, and the persisted flag matching a fresh recomputation on every row.

  The verdict is also stamped on every label as it is written now, not left to the reader: `MetaLabelDatasetBuilder` recomputes it, but it is not the only consumer — `heber/features/pipelines/ticker_base_rates.py` aggregates these rows straight off the lakehouse — so a label resolved against a short window says so itself.

  `BarrierChecker` evaluating snapshots outside the window (below) was the remaining gap here; those rows were flagged regardless (the verdict follows the window, not the outcome), but are now also computed correctly.

- **Label rows whose outcome predates their own alert no longer reach as-of reads** (`scripts/quarantine_inverted_label_windows.py`): 3,368 rows carried `ts_available < ts_event` by up to 3.5 hours, because an expiry's outcome time is its `window_end` and the timezone relabel put some window_ends before the alert that opened them. `HeberReader.read_asof` pushes `ts_available <= asof_time` into the scan, so a backtest asking "what did I know at T" was handed an outcome that had not been observed — and could not have been, its window having closed before it opened. All 3,368 are `expired`/`hit_sl`/`hit_tp` labels over a zero-or-negative window, so none carries a usable outcome. They are **moved, not deleted**, to a sibling `_quarantine/window_end_before_alert/` tree, with a full copy of every rewritten fragment kept alongside. Verified after the run: 191,262 canonical + 3,368 quarantined = 194,630, the original `watch_id` set exactly, no row in both trees, and **zero canonical rows still inverted**.

- **`read_gold` and `read_silver` no longer return quarantined rows** (`heber/reader/core.py`): `_collect_parquet_files` recursed the entire dataset root and filtered only `._*` sidecars and `.tmp` partial writes, so the sibling `_quarantine` tree came back with the data. Measured on `meta_label_features/version=v1`: **10,392 of the 10,512 files collected were quarantine files** — every all-Greeks-null row the writer deliberately withheld has been flowing back into training reads, which is exactly the corruption the quarantine exists to prevent. Directories whose name starts with `_` are now skipped in the recursive walk, evaluated relative to the requested root so a caller whose own path contains one still sees its data, and excluded outright when a caller addresses a quarantined file directly — no read path may reach quarantine, not even by naming the exact file. (Generalizes the dataset-specific `_quarantine` exclusion `_parquet_files_in` already had from the schema-conflict fix below — this also keeps `_locks` and any future underscore-prefixed convention out of every read path, not just quarantine.)

- **SWING and LEAP watch horizons are now stated in the units the calendar spends** (`heber/watch/models.py`): `max_duration_hours` was 120 and 720 — 5x24 and 30x24, calendar hours — but its only consumer passes them to `add_trading_hours`, which spends them at 6.5 hours per session. With the calendar repaired that made a SWING window 18.5 sessions (~26 calendar days) and a LEAP window 110.8 sessions (~5 months), both routinely outliving the contract being watched, since SWING alerts are 3-21 DTE and LEAP 22+. They are now 5 and 30 *trading* days. Rows already written carry `label_window_truncated`, and the builder combines the persisted flag with its recomputation, so the lower nominal does not un-exclude anything already marked.

- **Gold compaction and in-place partition rewrites no longer race** (`heber/gold/partition_lock.py`, `scripts/compact_gold.py`): compaction merges a partition's fragments and deletes the originals, so a concurrent fragment rewrite could resurrect a deleted fragment beside its compacted copy and duplicate every row in it. Both paths now take the same per-partition lock, held across the whole read-merge-delete. Lock files live in a sibling `_locks` tree — a zero-byte file next to the data breaks `pyarrow.dataset`'s auto-walk with `Parquet file size is 0 bytes`.

- **`BarrierChecker` no longer lets a snapshot collected after a watch's window closes decide its outcome** (`heber/watch/checker.py`): TP/SL barriers were checked over every stored snapshot before the window-expiry check ran, with no filter on `snapshot.timestamp <= window_end`. Because the check loop runs on an interval, a snapshot collected between `window_end` and the next check pass could cross a barrier and be reported as `hit_tp`/`hit_sl` with a barrier timestamp after the window the watch was ever authorised to run in — instead of the honest `expired`. The return path (and its MFE/MAE) is now filtered to snapshots within `[alert_time, window_end]` before barrier evaluation. 51 watches in Redis currently carry stale truncated `window_end` values from the pre-`b71d63e2` calendar bug and are past their window — exactly the population most likely to have tripped this.

- **A watch label that failed to write no longer disappears for good** (`heber/watch/manager.py`, `heber/watch/checker.py`, `heber/watch/writer.py`, `heber/watch/models.py`, `heber/ops/metrics.py`): `BarrierChecker` marks a watch complete and drops it from Redis's active set *before* the caller ever hands its outcome to `LabelWriter.write_outcomes`. If that write raised — a transient Parquet or filesystem failure — the outcome had only ever lived in a local variable for that one loop iteration, the watch was already gone from the active set, and nothing would ever recheck it: the label was silently and permanently lost, with just a log line to show for it. `BarrierChecker.check_all` now durably stages every completed outcome in Redis (`heber:watch:pending_outcome:{watch_id}`) the moment it's computed — the JSON write and its set-index entry go through one pipelined transaction so a crash can't leave one without the other — and `WatchService`'s check loop retries whatever is still pending on every tick, not just outcomes discovered that tick, clearing each one only once its Gold write actually succeeds. A pending outcome left over from a crash is picked up and flushed the same way on the next run. A pending record that fails to deserialize is now a loud `logger.error` plus a new `heber_watch_pending_outcome_corrupt_total` metric instead of a warning, since — same as the bug this closes — nothing else will ever revisit it once the watch is off the active set.

  **Follow-up — both residual gaps flagged by that review are now closed for the failure mode that actually matters here (a process crash or an ordinary Redis/filesystem error).** `WatchManager.complete_watch_and_stage_outcome` replaces the two separate complete-then-stage writes `BarrierChecker` used to make: completing the watch, removing both its indexes, and staging its outcome now happen in one Redis pipeline. If that transaction never lands, the watch comes back untouched — still `WATCHING`, still active — for the next checker pass to retry from scratch, instead of ending up complete with its outcome nowhere retryable. Separately, `LabelWriter` now writes one Parquet file per outcome, deterministically named by `watch_id` instead of a per-flush timestamp+random suffix: retrying an outcome that already wrote successfully (a crash between the write and clearing its pending entry) overwrites its own file with the same content instead of creating a duplicate row under a new name. Rolling back a failed flush now also leaves alone any file that already existed before that flush started, so an unrelated row failing later in the same batch can't delete a different row's already-durable data.

  A second adversarial review of this follow-up (verdict: needs-attention) surfaced two narrower gaps this does *not* claim to close, both requiring infrastructure this repo doesn't have today to close properly: Redis's `MULTI/EXEC` doesn't roll back a *runtime* command error (e.g. one queued command hitting `WRONGTYPE` after earlier ones already applied) — closing that fully needs a Lua script that validates key types before writing, which needs a real-Redis test fixture this codebase doesn't have (every watch test uses a hand-rolled Python fake); and the deterministic filename isn't safe under *concurrent* writers — a non-issue today since `docker-compose.yml` runs exactly one `heber-watch` container, but would need a distributed lease to stay safe under a future multi-instance deployment. The same review also flagged that one file per label is an unbounded small-file cost for Gold reads, given this repo's own measured ~106ms-per-footer mount cost — a real tradeoff, accepted for now rather than building a compaction pipeline as part of this fix; a training-window read pays a footer-open per completed watch instead of per flush-batch.
- **The reader no longer silently coerces a column whose type disagrees across partitions** (`heber/reader/core.py`): when fragments disagreed on a column's type in a way that wasn't a plain numeric widening or a string-encoding variant — a decimal price merged with a `double`, or the same real-world bug this closes, `expiry` written as `int64` in one partition and `string` in another — the reader used to fold the disagreement into whichever type pyarrow's schema inference happened to see first. Decimal-into-double silently drops precision above 2**53; string-into-numeric silently changes what the column *is*. Neither raised: whether it did depended on filesystem enumeration order, so the same feed could read cleanly on one host and quietly corrupt values on another — including a `string -> int64 -> string` sequence where the oldest and newest partitions happen to agree and only the middle one drifted. These now raise `SchemaContractError` instead, matching the tz-mismatch guard this reader already applies to point-in-time columns; catching a middle-fragment drift needs every fragment's schema, not just the endpoints, so `read_gold` and `read_parquet_dataset` (the path `MetaLabelDatasetBuilder` reads Gold through) now unify unconditionally rather than trying to guess first whether a cheaper check would do — the two-endpoint probe this replaced is gone, since a real fail-closed guarantee can't be built on a heuristic that assumes drift is monotonic. `ts_label`, the `time_range` column on datasets without `ts_event`, now gets the same fail-closed treatment as the other point-in-time columns for a timezone mismatch — it drove a predicate just as directly as they do but wasn't included.

  This guarantee is deliberately scoped to `read_gold`/`read_parquet_dataset`. `read_silver`/`read_asof` — Heber's highest-volume, most latency-sensitive read path, hit routinely by live trading systems and usually called without date-scoping — keeps the previous reactive-only recovery for now; extending the same unconditional per-fragment validation there risks multi-minute latency on a hot path for a comparatively rare corruption case. Tracked in [#50](https://github.com/JasperDale420/Heber/issues/50), which needs a production-scale benchmark and a cheaper detection mechanism (e.g. a per-file validated-schema cache) before it's safe to close.
- **`read_gold(version=None)` no longer pulls every retired version into schema validation** (`heber/reader/core.py`): resolving "latest" scanned the `dataset=/project=` parent so hive partitioning would expose the `version=` column — needless, since pyarrow reads that column from each file's full path regardless of which directory is scanned — which meant a schema change between v1 and v2 (ordinary evolution) could fail or force a full-dataset schema union on a "give me latest" read that never asked for the old version. Combined with the fail-closed change above, an incompatible retired version could now break an otherwise-healthy current-version read outright. `version=None` now scans the resolved version's directory directly.

- **Every Gold feature write now records its own enrichment provenance** (`heber/ml/datasets.py`): the freshness gate refuses stale live-only enrichment, but it can be switched off with `HEBER_WATCH_LIVE_ENRICHMENT_MAX_AGE_MINUTES=0` for a deliberate backlog replay — and that path wrote present-day Greeks onto past alerts with nothing marking them. Only rows that already existed had been flagged, by a one-shot migration. `persist_features_to_gold` now flags at write time, where `ts_available` is authoritative, so no future write can land unflagged. Rows already in a partition keep the flags they were written with.

  Disabling the gate does **not** disable the flagging: the bound falls back to the setting's own default rather than becoming zero, which would otherwise have marked every row late — live writes included — in exactly the configuration that lets stale enrichment through. `scripts/flag_legacy_enrichment.py` shares the same bound.
- **Bad expiry dates could sneak into the options data looking clean** (`heber/ml/datasets.py`, `scripts/normalize_gold_expiry.py`, `heber/watch/consumer.py`, `heber/watch/backfill_scanner.py`): a corrupted or malformed expiry date — extra stray characters, a decimal that shouldn't be there — used to get silently trimmed down and accepted as if it were a real date, with nothing marking the row as suspect. That's now caught and rejected everywhere an expiry date can enter the system: the shared conversion used by the main data pipeline and the cleanup script, the live alert-processing path, its days-to-expiry calculation, and the overnight re-enrichment job (which also used to guess "today" for a missing expiry instead of skipping the row). A date built from certain non-English digit characters could still slip past the first fix — closed too.
- **A rare disk hiccup could leave a corrupted, empty options-data file that looked fine** (`heber/ml/datasets.py`, `scripts/normalize_gold_expiry.py`): if the machine lost power or a process was killed at exactly the wrong moment during a write, the file left behind could appear present but actually be empty or broken — the storage volume this data lives on doesn't have the safety net most drives do. Writes to this dataset are now more careful about confirming the data really reached disk before considering the write finished. Several related rough edges found while fixing this: a failed write for one day of data no longer throws away progress already made on other days in the same batch, and the caller is now told when that happened instead of the batch quietly looking like a full success; a failed verification during the cleanup script's rewrite now keeps the broken file around to diagnose instead of deleting the only evidence of what went wrong, and that kept evidence is itself now written to disk properly instead of risking the exact same "looks present but isn't really there" problem this whole fix exists to close.

  One deeper gap surfaced during review and is intentionally not part of this fix, since closing it properly means changing code shared by several other parts of the system rather than a targeted change here: on this Mac, even a "successful" disk flush isn't always as durable as it would be on Linux, for storage attached the way this volume is ([#55](https://github.com/JasperDale420/Heber/issues/55)).

- **A re-delivered flow alert no longer creates a second watch** (`heber/watch/manager.py`, `heber/watch/consumer.py`, `heber/watch/models.py`): watch creation minted a fresh `watch_id` for every delivery of an alert, with nothing keyed on `alert_id`. Each extra watch ran to completion and wrote its own Gold `labels_alert_barriers` row, so an upstream re-delivery storm on 2026-03-12 (2,730,333 Bronze records for 8,567 distinct `event_id`s; ~200 alerts re-delivered ~13,600 times each) left 194,630 label rows for 149,289 distinct alerts — 45,341 duplicates, one alert holding 918 rows. Because the fan-out was uneven across dates, affected alerts were massively overweighted in training loss. Creation now claims `heber:watch:by_alert:{alert_id}` with an atomic `SET NX`, so a burst of concurrent duplicates cannot race into two watches. The claim is taken after all validation, and a failure to persist rolls the half-written watch out of the active set before releasing it, so a Redis failure mid-create leaves the alert retryable instead of ACKing it away as a phantom duplicate or leaving two active watches. If the rollback itself cannot reach Redis the claim is kept — losing one label beats admitting a duplicate one, the same tradeoff taken when a fatal gateway auth failure kills feature extraction after the watch exists: the alert stays claimed, so that watch's label is simply dropped by the training join instead of a second watch being created for it. Its 270-day TTL clears the longest possible window with margin — a LEAP watch runs 30 *trading* days (195 trading hours after the horizon-units fix above; 720 hours and ~111 sessions / 156 calendar days before it, which this TTL was originally sized for). Duplicates are skipped before entry-price resolution and feature extraction, reported as a `duplicate_alert` outcome in consumer logs, and counted by the new `heber_watch_duplicate_alerts_suppressed_total` metric — the signal that would have made the March storm visible on day one. The claim is deliberately *not* released when a watch completes: an alert that has already been labelled must never be re-watched. Existing duplicate rows are left untouched; deduplicating them is a separate labelling decision.
- **The silver compactor no longer walks Atlas's `massive_*` trees** (`heber/writer/compactor.py`): the hourly scan treated every top-level silver dataset as Heber partitions, so it created and removed a `.compaction.lock` inside each of `massive_bars`' 11k one-file directories every cycle (holding handles inside Atlasv2's cache-swap transaction, which correctly rolled back), had merged per-ticker files in `massive_taq_secagg_1s/2024` into `compacted-*.parquet`, and would have merged the flat `massive_intraday_daily/year=*.parquet` layout that Atlas reads by file name. Top-level datasets named `massive_*` are now reserved from generic compaction — never listed, locked, or merged; Heber's own feeds compact exactly as before.
- **The coverage walk no longer stats every Parquet file, so a pass can finish** (`heber/catalog/seeds.py`): the zero-byte check ran on every file on every pass, including the ~95% a reuse pass never opens. Measured on the lakehouse mount: `scandir` alone manages 6,669 files/sec, `scandir` plus a per-file `stat()` manages 228 — 29x slower, 4.24 ms of pure stat per file. Parallelism is per directory, so one leaf holding ~800 files serialised 800 stats while occupying a worker. `feed=quotes` at ~825k files had therefore never once completed a walk in any container, which left roughly 21 feeds after it alphabetically permanently unscanned. The check now runs in `_read_row_count`, immediately before the footer read, where it is paid only for files being opened anyway and 4 ms disappears beside a 106 ms read. Zero-byte files are still never handed to pyarrow, and are still counted apart from unreadable ones.

- **SWING and LEAP watch windows are no longer collapsed to a few hours on the alert day** (`heber/calendar/market.py`): `MarketCalendar.add_trading_hours` could not cross a session boundary. `_next_session()` returns the *current* session for any timestamp before that session's close, so the "use up this session and continue" branch reset the cursor back to the same day's open and burned 390 minutes per iteration against the start date forever — a 720 trading-hour (LEAP) span ended up at `open + (43200 mod 390) = open + 5h`, and a 120-hour (SWING) span at `open + 3h`, both on the alert day. Separately, the final `.replace(tzinfo=UTC)` relabelled an America/New_York timestamp as UTC instead of converting it, shifting any result that stayed inside one session back by the ET offset. Both are fixed: the next-session search now starts from the session close, and the result is converted with `tz_convert`. `trading_minutes_until` carried the same next-session defect and would spin on any span crossing a session boundary; it is fixed the same way. A span that ends exactly at a session close (13:00 ET + 120h, 11:00 ET + 720h, and any full-session span on an early-close day) now resolves to the next session's open instead of the close itself, which is a minute the market is shut — previously that could land a window on a Friday evening and expire it across the weekend.

  Blast radius in the live watch set: of 28,682 watches in Redis, **14,796 SWING watches had a median window of 3.01 hours instead of ~120 trading hours, and 13,657 LEAP watches had 5.01 hours instead of ~720** — so nearly every `expired`, `hit_tp` and `hit_sl` label on those horizons is really a same-day intraday outcome, and the SWING/LEAP/INTRADAY horizon distinction was meaningless.

- **`heber-watch` no longer silently discards afternoon alerts as "stale"** (consequence of the calendar fix above): `_process_parsed_alert` drops an alert when `window_end <= now`, ack'ing the message as a non-retryable success with only an INFO line. Because the broken window landed *before* the alert time for late-session alerts, every INTRADAY alert fired during regular hours was dropped (all 390 session minutes), along with SWING alerts after 13:01 ET (179 of 390 minutes) and LEAP alerts after 11:01 ET (299 of 390 minutes), in both EST and EDT. The surviving watches match exactly: all 229 INTRADAY watches in Redis have an alert time after the 16:00 close, SWING watches stop at ET hour 12, and LEAP watches at hour 10. No session minute is dropped after the fix.

- **`/health/coverage` now names the feed that has gone longest without a scan** (`heber/catalog/api.py`, `heber/catalog/seeds.py`): it reported `max(last_updated_ts)` across every row, which only ever meant *something* was written recently — one feed could satisfy it on behalf of all the others. That is how the stalled walk hid: it never got past `feed=quotes`, so every feed after it alphabetically went unscanned while the endpoint read fresh. `stalest_feed` and `stalest_feed_age_seconds` now report the oldest per-feed scan alongside the status.
  Status still gates on the newest scan. Gating on the oldest is the honest behaviour but cannot be switched on yet: `_walk_partition_files` enumerates a feed's whole tree every pass regardless of reuse — reuse only skips reading footers — so `feed=quotes` at ~825k files has never once completed a walk in any container, and gating on the oldest would report stale permanently. Pruning that walk is what unblocks it.
...
- **The enrichment backfill scanner can no longer downgrade a good feature row** (`heber/watch/backfill_scanner.py`): the hourly scanner selected any row with at least one null enrichment field and replaced it wholesale with a freshly extracted row. Since the freshness gate landed, a fresh extract for an alert older than `HEBER_WATCH_LIVE_ENRICHMENT_MAX_AGE_MINUTES` (default 60) returns null for every live-only field — so a row captured live at alert time with valid Greeks, missing only something unrelated like `max_pain_strike`, had its Greeks destroyed and replaced with nulls. The patch is now strictly additive: it fills only the enrichment fields the row is actually missing and never overwrites a value already present. Selection also skips any alert past the age bound (a fresh extract cannot contribute anything for those), skips everything when the age gate is switched off, and re-checks age immediately before extraction, since a full batch takes minutes of gateway calls to work through and a row can cross the bound while queued. `ts_available` advances when a value is filled, so a value fetched now is not backdated to the original write. A survey of 117,824 rows across 120 partitions found no row that had already lost its Greeks this way.

  The patch is applied in place under the same partition lock the canonical writer uses, and re-checks nullness against the on-disk row rather than the scan snapshot — a live write for the same `alert_id` landing in between is no longer reverted.
...
- **The three places that still turned an unreadable read back into "no data"** (`heber/reader/core.py`, `heber/gold/labels.py`, `heber/ml/datasets.py`, `heber/health_monitor/checks/ml_readiness.py`): making `HeberReader` raise on unreadable files only helps where the caller lets the error out, and three consumers immediately caught it and returned an empty frame again. `read_parquet_dataset` raised when the *scan* failed but not when the *open* did, so a file that failed one step earlier still read as empty — it now applies the same split as the Silver and Gold reads, where files that are present but unopenable raise and only a genuinely empty walk returns empty. `read_label()` and the meta-label dataset builder's two loaders each wrapped their read in `except Exception` and returned an empty frame, so a corrupt label or feature file produced "No outcomes found in date range" — the same message a date range that genuinely holds none produces — and training proceeded against it.
- **A corrupt Gold dataset no longer leaves the ML readiness report green** (`heber/health_monitor/checks/ml_readiness.py`): `_safe_read_gold` has always separated a failed read (`None`) from a successful empty one, and its docstring says callers must not report a clean PASS for a read that did not happen — but only the zero-leakage audit acted on it. Label stability and feature nulls grouped `None` with "no data today" and returned **PASS**, and cross-sectional completeness skipped it silently, emitting no result at all. All four now report `ERROR` naming the dataset they could not read, so one unreadable file can no longer pass three guardrails.
- **One damaged Bronze file no longer makes a whole day un-replayable** (`heber/writer/transformer.py`): replaying Silver from Bronze aborted outright on a Bronze file that had been truncated by an unclean shutdown — `quotes/dt=2026-06-18` holds 1,839 files and one bad one stopped all of them. `gzip` reports a torn stream as `EOFError`, a damaged block as `zlib.error`, and bytes that inflate to non-UTF-8 as `UnicodeDecodeError`; none is an `OSError`, which was all the reader caught, so each escaped mid-iteration and discarded every record already decoded from that file as well. A damaged file now gives up the records it still holds and the rest of the date is replayed. **This is the one layer where damage is permanent** — Bronze is what everything else replays from, so what a truncated file lost is gone.

  Guards that came out of reviewing the above:
  - A replay that cannot read **more than half** the files for a date now raises instead of returning a count. A dropped mount fails every file identically, and the previous behaviour was to log each one and report `records=0` with a clean exit.
  - I/O errors are logged as `bronze_io_error` separately from content damage (`bronze_file_truncated` / `bronze_file_corrupt`), so a failing volume is distinguishable from a handful of bad files.
  - Content that inflates but **fails its CRC** is discarded rather than salvaged: a flipped digit still parses as valid JSON, and passing those rows off as recovered data would launder corruption into Silver.
  - `._*` AppleDouble sidecars are skipped instead of being reported as damaged Bronze, and zero-byte Bronze files — which otherwise read as cleanly empty and are never mentioned — are logged.

- **`heber-consumer` and `heber-watch` now have memory ceilings** (`docker-compose.yml`): both ran unbounded, and a container with no cgroup limit is killed at the VM level with no `OOMKilled` flag and no exit code — the week of Jul 6 lost 32 health-monitor and 6 compactor deaths that way, unattributable. The consumer gets the same 3g as `heber-backfill-consumer`, which runs the same writer code and the same double-buffering; watch gets 3g against an observed ~960MB peak. These are for attribution, not enforcement — sized well clear of observed usage so they do not cause kills that would not otherwise happen. If `OOMKilled` ever reads true on either, raise the ceiling rather than assume a leak.

- **A lakehouse-volume outage now pages once instead of once per container** (`heber/ops/stack_check.py`): every heber service bind-mounts the volume, so when it disappears all nine fail at once and the alarm fired a separate critical for each — 22 results for one incident. The volume alert now says so and the downstream container failures are folded into it. `data-gateway` and `data-gateway-redis` are still reported (they do not use the volume, and Redis is what buffers live events while Heber is down), and a *paused* container is still reported too, since nothing about an absent mount pauses a container. Containers nobody has classified are reported rather than suppressed.

- **The alarm's volume check no longer accepts an empty placeholder directory** (`heber/ops/stack_check.py`): it tested `data_root.is_dir()`, but a broken mount can leave the mount point behind as an empty directory — exactly the case it exists to catch. It now checks the `.heber-sentinel` file, the same signal `heber/writer/compactor.py` and `heber/ops/dataflow_health.py` already use, and treats the EPERM a zombie mount raises as "absent".

- **A consumer that is alive but not ingesting is now alerted on** (`heber/ops/stack_check.py`, `heber/ops/metrics.py`): new `stack_ingest` check covering **both** `heber-consumer` and `heber-watch` — the two services that were down together in the 08-08 and 08-11 episodes and that now wait Redis out rather than crash-looping, so neither announces a stall by dying any more. It scrapes each one's last-successful-`XREADGROUP` gauge from its loopback metrics port and pages when no read has succeeded in 10 minutes. The container healthcheck cannot see this — it reads the run-loop heartbeat, which stays fresh while the loop spins on a dead upstream. The 10-minute bound is deliberate: the gauge advances when a read *returns*, so its gap also spans processing of the batch it returned, and a large batch fanned across many partitions has outrun the 180s healthcheck window before. A container built before the gauge existed, or one still on its first read, warns instead of paging, and a consumer already reported down is not paged about twice.

- **The docker watchdog stops thrashing when the lakehouse volume is missing** (`scripts/heber_docker_watchdog.sh`): every heber service bind-mounts `/Volumes/heber`, so while it is absent Docker fails the mount itself and `compose up` cannot start anything — after the 2026-08-12 boot the exFAT volume was under repair until 10:14:17Z and the watchdog burned 35 ticks and 89 `mkdir /host_mnt/Volumes/heber: permission denied` errors against it, repairing nothing. It now checks for the volume first and leaves the heber services alone until it returns. The upstream (`data-gateway`, `data-gateway-redis`) and unpausing are still handled, deliberately — those do not need the volume, and Redis is what buffers live events while Heber is down. Presence is tested by the `.heber-sentinel` file rather than the directory, because an unmounted volume can leave an empty placeholder directory behind that a directory check accepts.

- **An upstream Redis restart no longer crash-loops the whole stack** (`heber/writer/consumer.py`, `heber/watch/consumer.py`): both services retried Redis forever *inside* their run loop, but created their consumer group *before* entering it — so a Redis that was down or still loading its AOF killed the process in `xgroup_create`, and `restart: always` turned that into a crash-loop the watchdog restarted every 120 seconds against the same dead Redis. `data-gateway-redis` takes ~77 seconds to load its 797k-key AOF, so every restart of it triggered this: 1h38m of it on 2026-08-08 and 25m on 08-11, ~43 watchdog interventions that fixed nothing. Startup now backs off and waits the upstream out. Genuine misconfiguration still fails fast — only transient errors are retried.

- **A consumer stalled on Redis is no longer reported as healthy** (`heber/ops/metrics.py`, `heber/writer/consumer.py`): the healthcheck reads the run-loop heartbeat, which is refreshed at the top of *every* iteration including ones that caught a Redis error and slept — so a consumer spinning against a dead upstream looked healthy indefinitely. Without this, the retry fix above would have traded a loud crash-loop for a silent ingest stall. A new `heber_consumer_last_xread_success_unixtime` gauge advances only on a successful `XREADGROUP` round-trip (an empty read still counts — it proves Redis answered), so "spinning" and "consuming" are finally distinguishable. It complements `heber_consumer_acked_total`, which cannot separate "idle, nothing to read" from "Redis is gone".

- **Historical rows whose enrichment was not point-in-time are now flagged** (`heber/ml/datasets.py`, `scripts/flag_legacy_enrichment.py`): before the freshness gate, `EnrichmentBackfillScanner` re-enriched rows up to three days old every hour, refetching Greeks, `iv_rank` and `max_pain` live and writing present-day values onto past alerts. Nothing distinguished those rows by inspection. A one-shot migration flagged **9,892** rows `enrichment_captured_late` and **21,510** `enrichment_provenance_unknown` across 116,343 rows in 66 partitions, adding only to `quality_flags` — no row was moved, added, or dropped, and row count was verified identical before and after. The flag keys on carrying live-fetched enrichment, not on lag alone, so the deliberately null-enriched 2026-08-06/07 backfill is not mislabelled. `ts_available - alert_time` is an upper bound on capture staleness (it spans enrichment plus the write), so the flagged set is a superset; both columns remain on every row for a training run to re-threshold. Each run writes a manifest recording the bound and per-partition counts.

  Three partitions (`dt=2026-04-14`, `dt=2026-05-06`, `dt=2026-06-17`) could not be processed — they hold out-of-range timestamps (`year must be in 1..9999, not 57442`) that make them unreadable to pandas, so they are already invisible to training. The migration exits non-zero rather than reporting success.

- **Alert feature enrichment no longer stamps present-day market state onto past alerts** (`heber/watch/features.py`, `heber/config.py`): the gateway routes behind Greeks, IV rank, GEX, max pain and market tide all answer "as of now" and accept no as-of timestamp, so running them against an old alert wrote today's delta/gamma/theta/vega/iv onto a days-old observation — look-ahead in a dataset that feeds model training. `AlertFeatureExtractor` now refuses those steps for an alert older than `HEBER_WATCH_LIVE_ENRICHMENT_MAX_AGE_MINUTES` (default 60) and records why in the new `quality_flags` column instead. Dated stock bars (returns, realized volatility) are unaffected and still enrich correctly for any past date. This closes the leak in three places at once: the manual backfill script, the hourly `EnrichmentBackfillScanner` that was re-enriching up to 3 days back inside the live watch service, and `heber/ml/inference.py`. Set the bound to `0` only for deliberate replay of an ingest backlog.

- **Backfilled feature rows now match the live writer's column set** (`scripts/backfill_features.py`, `heber/watch/backfill_scanner.py`, `heber/watch/features.py`): the backfill script wrote `AlertFeatures.__dict__` directly and the enrichment scanner wrote `to_dict()`, both keeping the internal `enrichment_failures` field, and the script also omitted `instrument_key`, `ts_event` and `ts_available` — so partitions written by different writers were schema-divergent and the columns nulled each other out on concatenation. All three writers now go through the shared `feature_row_for_gold()`.

- **`ts_available`, `ts_event` and `instrument_key` no longer enter the model feature matrix** (`heber/ml/datasets.py`): `get_feature_columns()` excluded identifiers and timestamps by name but missed these three, so every meta-model trained through `to_xy()` received a string instrument key and two timestamps as inputs. `ts_available` is the write time — feeding it to a model leaks exactly the write lag that separates a live capture from a late backfill.

- **`scripts/backfill_features.py --fill-nulls` works again, and can be scoped to specific dates**: it opened the partition tree with a raw `pyarrow.dataset()` walk, which picks up the zero-byte `data.parquet.lock` file beside each partition and failed with `Parquet file size is 0 bytes`. It now reads canonical `dt=` partitions by explicit path — which also keeps the `_quarantine/` tree out of the read — and honours `--dates` so a single day can be filled without loading all 119 partitions.

- **Market-tide recovery from Silver had never worked, and its net-premium formula was wrong** (`heber/watch/features.py`): `_backfill_market_tide` asked Silver for `net_call_premium`/`net_put_premium`, but the ingest contract maps those provider fields to `total_call_premium`/`total_put_premium` on write, so the columns came back absent and the join raised `KeyError` whenever tide data was actually present. It now reads the mapped names. Separately, it computed net premium as `net_call - net_put`; both legs are already *signed* net flows, so subtracting added the put magnitude instead of cancelling it — producing a value that was never negative (+27M to +176M on 2026-08-06) and disagreed with the provider's own `sentiment` on 58 of 64 bars, against a live-captured range of −15.7M to +2.8M. It now sums the signed legs, matching the convention already used for `vex`.

- **Appending a provenance flag no longer erases the flags already on the row** (`heber/watch/features.py`): a Parquet list column reads back as `numpy.ndarray`, not `list`, so a list-only type check saw no existing flags and replaced them. That silently stripped `greeks_no_point_in_time_source` from every row it touched, which would then have been diverted to the all-Greeks-null quarantine instead of the canonical partition.

- **GEX/VEX recovered from Silver is now flagged** `gex_recovered_from_silver`, matching the market-tide treatment — a daily-grain snapshot is a different feature from the live per-alert fetch and should be distinguishable in training.

- **`backfill_uw_fields` no longer fails on timestamp resolution** (`heber/watch/features.py`): the asof joins raised `MergeError: incompatible merge keys ... datetime64[ns, UTC] and datetime64[us, UTC]` when the Gold frame came from Parquet (microseconds) and the Silver frame from the reader (nanoseconds). Both sides are now normalized before the merge.

- **`scripts/backfill_features.py --overwrite` can now repair existing feature rows**: the script subtracted already-present `alert_id`s from its work list, so a row written with wrong enrichment could never be replaced — including the 12 rows in `dt=2026-08-07` whose Greeks were fetched 65 hours after the alert. The Gold write already dedups on `alert_id` keeping the last row, so re-extraction replaces them.
- **The dataflow-health tests no longer depend on the machine running them** (`tests/test_dataflow_health.py`): six report-shape tests asserted `overall_status == "ok"` while three of the probes composing that report inspected the host — `_mount_liveness_check` stats the lakehouse sentinel, `_catalog_health_check` and `_catalog_coverage_check` make real HTTP calls to the catalog. On a developer machine with the volume mounted and the catalog running they returned `ok` and the tests passed; anywhere else — CI, or a laptop with the volume unmounted — all three returned `fail` and every assertion collapsed. That kept CI red on every open PR. Those probes are now stubbed for the report-shape tests and given dedicated hermetic coverage of their own, so the host coupling is gone without losing what they verify.
...
- **The alert job can now report its own death** (`heber/ops/heartbeat.py`, `heber/cli.py`, `HEBER_ALERT_HEARTBEAT_URL`): every check the alarm runs lives on the machine it watches, so it could say nothing about being unloaded, wedged, killed by a broken `uv.lock`, or the host simply dying — the one failure that silences everything at once. `alert-check` now pings an external check on every cycle, and the *absence* of pings is what raises the alarm. `/fail` is appended when the run crashed or a Discord send failed, so a live-but-broken job alerts immediately instead of waiting out the grace period. A revoked or rotated webhook is the case that matters most: alert sends are swallowed by design so a broken webhook cannot crash the monitor, which until now meant every alert could vanish with nothing to show for it.
  This must be a **different** check from `HEBER_HEARTBEAT_URL` (dataflow-health's): sharing one would let a live dataflow-health mask a dead alert-check. Both are empty by default and disabled until set — see `.env.example`.

- **The stack outage alarm now runs outside the stack** (`heber/ops/stack_check.py`, `heber/cli.py`): every failure detector Heber had ran as a container in the stack it was watching, so an outage that took the stack down took the alarm with it. On 2026-08-12 a Docker Desktop restart left `data-gateway` and `data-gateway-redis` stopped while three consumers crash-looped ~105 times each against the dead Redis for 1h40m, ingesting nothing — `heber-dataflow-health` was checking the Redis connection every five minutes throughout and could not say so, because it was a container in the same stack. The native `alert-check` job, which already ran on the host, now also reports: any expected container stopped, missing or **paused** (paused containers have been observed still showing `Up` while processing nothing), Heber's upstream `data-gateway`/`data-gateway-redis` being down, a container restart-looping, `/health/coverage` reporting stale, the lakehouse volume being unmounted, and the Docker daemon being unreachable. When `heber_docker_watchdog.sh` has repaired something, that is reported too, so a stack being propped up every two minutes is visible rather than invisible.
  Nothing new is scheduled — this extends the existing launchd job rather than adding one. A dead Docker daemon reports once rather than fanning out into an alert per container, and an unmounted volume suppresses the coverage alert it would obviously cause. Restart-looping is read from Docker's own `restarting` state plus a recent-start check, because `docker compose up` resets `RestartCount` to zero and a container that restarted often but has been stable for hours has recovered.
...
- **The catalog coverage scan no longer takes longer than the alarm watching it** (`heber/catalog/seeds.py`, `heber/config.py`): one pass took **8h34m** — the container started at 21:34:30 and `coverage_scan_complete` landed at 06:08:43 — against a `HEBER_CATALOG_COVERAGE_MAX_AGE_SECONDS` of 6h. A pass that takes longer than the staleness threshold can never satisfy it, so `/health/coverage` was going to report stale between every pass and alert on a healthy system, and a container restart guaranteed 8½ hours of it. The alarm was correct; the thing it measured was too slow. Three consecutive passes on the deployed build now take **2h15m, 2h31m and 2h38m**, and the change below takes that down again to the minutes a pass spends on partitions that actually changed.
  Timing each phase showed the database was not involved: writing all 16,681 coverage rows took **6.6 seconds**. The entire cost was the disk walk, and on the exFAT lakehouse mount that cost is per-open latency — measured at ~28ms to open a directory and ~106ms to read one Parquet footer — not bandwidth or CPU. Three things followed from that. The walk visited every partition twice, once to find `dt=` directories and again to list the Parquet files inside each; it now makes a single pass. Directory listings and footer reads are issued concurrently rather than one at a time, since latency overlaps where CPU would not — the mount only parallelises about fourfold before it serialises, so `feed=bars` now costs 139s for its 7,706 directory and footer opens where the same opens issued one at a time cost roughly 550s. And zero-byte Parquet files are skipped by size instead of being opened — 175 of them exist, 173 under `feed=quotes`, and each one cost an open, a raised `ArrowInvalid`, and a full stack trace rendered into a JSON log line on every pass. The compactor and `HeberReader` already skipped empty files this way.
  Each feed now logs its own `dirs`, `files`, `skipped_empty`, `unreadable` and `elapsed_seconds` as it finishes, so the next time a pass gets slow it is clear which feed owns it rather than being a single number for the whole lake. `unreadable` is there because a footer that cannot be read contributes no rows exactly like an absent one: if a degraded mount started failing a fraction of reads, per-date counts would quietly shrink, and that count is what makes it visible. `HEBER_CATALOG_COVERAGE_SCAN_WORKERS` (default 16) tunes the concurrency; it and `HEBER_CATALOG_COVERAGE_MAX_AGE_SECONDS` are now passed through in `docker-compose.yml`, which previously listed only three env vars for this service, so setting either had no effect on the container.
- **The coverage scan now costs what changed, not what exists** (`heber/catalog/seeds.py`): the per-feed timings the change above added immediately named the culprit — `feed=quotes` holds **824,928 Parquet files** across 4,445 directories, and reading every footer took **7,918s of a ~9,100s pass, about 88% of it**. Parallelism halved that; it cannot fix re-reading 825k unchanged files every five minutes. The scan now records each `dt=` partition's newest directory mtime as it walks, and skips the footer reads for any date whose directories have not been touched since that date's coverage row was last written, carrying the recorded count forward. Nothing in Heber rewrites a Parquet file in place — Silver writes new part files and the compactor writes a temp file then renames — so a directory's mtime is a sound signal that its contents changed, and a backfill writing into a historical date is picked up like any other change. Each feed reports `reused_dates` alongside its other counts, so the proportion of a pass that was genuinely re-read is visible.
  The saving is in the footer reads; the walk still visits every directory and checks every file's size, so a pass does not become free, it becomes proportional to what it has to open rather than to what exists.
  Coverage rows are now stamped with the moment the pass **started** rather than the moment they were written. Rows land minutes after the walk, so a write-time stamp would sit after files that arrived mid-pass; the next pass would see an older directory, skip it, and keep the short count permanently. A start-time stamp cannot go stale that way, and a 60-second margin absorbs the ~0.5s the mount's clock was measured running ahead of the container's. `/health/coverage` reads the same column, so its age is now measured from the start of the last completed pass — slightly more conservative, which is the right direction for a staleness alarm.
  **The first pass of each container counts everything from footers**, ignoring what is recorded. Rows written before this existed were stamped at write time, hours after the files they counted, so trusting one would permanently skip any partition whose last write landed mid-pass — an undercount nothing would heal, because no future write bumps the mtime. Recounting once per container lifetime also bounds the damage of any future mistake in the comparison. Only `dt:` rows this scan writes are ever reused; coverage rows written by backfill and `CatalogService.update_coverage` are still write-time stamped and are excluded.
  The one statement this needs — reading the recorded counts — runs before the walk and its transaction is closed immediately, because holding it open across a multi-minute walk is exactly the idle-in-transaction failure that froze coverage for 24 days.
- **The compactor now reaches `feed=quotes`** (`heber/writer/compactor.py`): it never had. Not once had a scan cycle run to completion, and `feed=quotes` had never appeared in a single compactor log line — with no crash, no restart and no memory pressure to show for it, which is why nothing flagged it. Two costs scaled with the number of files rather than the number of partitions. The scan walked with `rglob("*")`, which yields every *file* and then asks each one whether it is a directory — one stat per file, measured at 8 entries/s, so walking past that feed's 825,080 files would have taken about 28 hours. And selection stat'd every file in a partition before merging any: exFAT keeps no directory index, so a stat inside the largest directory — 35,955 files — measured 53.7ms, about 32 minutes to choose one batch, paid again on every cycle because only a capped batch is merged per visit.
  The walk now reads directory entries directly, which answers "is this a directory?" without a syscall per file, and selection stats only as far as the batch it is about to merge. It reaches `feed=quotes` 0.5 seconds into a cycle instead of never. The walk is breadth-first, as the old one was, so no single deep dataset can monopolise a cycle — a depth-first version spent 420 seconds without reaching one `feed=quotes` partition.
  The layer root is deliberately never treated as a partition. `rglob` never yielded the directory it started from; a walk that does would let two stray `silver/*.parquet` files be merged into one and the originals deleted, since compaction removes the files it merges.
  This does not finish the job: draining the largest partitions still takes many visits, and a full cycle still enumerates every partition three times. But the feed is now reachable, which it was not.
- **The underlying condition is only now being worked off**: `feed=quotes` accumulated 825k un-compacted part files while nothing was compacting it, and every `HeberReader` read of that feed pays for them too.
- **One unwalkable feed no longer discards the whole coverage pass** (`heber/catalog/seeds.py`): the scan collects every feed into a manifest before writing any of it, and any error other than a per-directory `OSError` escaped the whole collection loop — so an error on the first feed threw away the thirty behind it, and coverage stayed frozen until the staleness alarm fired, which is the outcome the scan exists to prevent. A feed that cannot be walked is now logged as `coverage_feed_scan_failed` and skipped, and `coverage_scan_complete` carries a `failed_feeds` count. This matches what the write phase already does, where each feed commits on its own so a late failure keeps the feeds already recorded.
- **A date's row count now covers the whole day, not one instrument type** (`heber/catalog/seeds.py`): when a feed held the same date under more than one `instrument_type=` directory — `bars` and `trades` both do, across equity, option and crypto — the scan emitted that date once per instrument type. Each emission wrote the same `(dataset_name, instrument_key)` pair, which `uq_dataset_instrument` makes unique, so the second write updated the first and the stored count was whichever instrument type happened to be walked last. The date is now summed across instrument types before it is written, so per-date counts for those feeds will increase after the first pass.
...
- **A failing lakehouse read can no longer mute the stack alarm** (`heber/cli.py`): the per-feed liveness checks and their `HeberReader` ran first and unguarded, so a volume, pyarrow or partition error took the whole `alert-check` run down with it. The stack checks now run first and depend on nothing but Docker and one HTTP call; a liveness failure is logged and the run continues.

- **A date range with no matching partition no longer reads the whole feed** (`heber/reader/core.py`): when a scoped read's range matched no `dt=` partition, the walk fell back to collecting every file in the feed. Asking production `feed=bars` for a month before its data begins collected all 5,967 files in 165.7s instead of none — now 0 files in 11.2s. Results were never wrong, because the scan predicate dropped the rows afterwards; the cost was a full-feed walk for every empty chunk of a chunked historical read, which is what stalled the market_regime full-history rebuild. The fallback still applies to datasets that have no `dt=` partitioning at all, which is what it was for.
- **A single unreadable file no longer makes a whole feed look empty** (`heber/reader/core.py`): one 0-byte `feed=darkpool/.../dt=2026-08-07/part-*.parquet` — a writer killed mid-write during one of the Docker outages — made every read of that feed return nothing. pyarrow raises `Parquet file size is 0 bytes` when it opens the dataset, and the reader caught that and returned an empty frame with only a warning, so months of healthy partitions became invisible and the darkpool regeneration reported `no_data` and exited cleanly. Zero-byte files are now skipped during the walk, the same way AppleDouble sidecars and `.tmp` partial writes already were, and the same way the compactor has always skipped them.
  More importantly, a read that fails now **raises** instead of returning an empty frame. Any other kind of corrupt file — truncated after a few bytes, a bad block — was equally capable of emptying a feed, and an unreadable partition was indistinguishable from an unwritten one. Absence still returns empty, because a partition nobody has written is a legitimate answer; unreadable bytes are not. Callers that previously carried on with silently incomplete data will now fail loudly.
- **A read scoped to a single day no longer comes back empty** (`heber/reader/core.py`): reading `feed=darkpool` for `dt=2026-08-07` with `prune_by_dt=True` returned 0 rows, while the same date unpruned returned 178,153 and a three-day pruned range returned 354,850. When a scoped walk narrows to one `dt=` directory, every file shares it, and the schema-unification path derived its partition root from the common prefix of the file list — so no `dt=` segment survived and the schema it built contradicted the partitioning passed with it. Partition columns are now taken from the directories the files sit in. Only feeds whose fragments disagree on a column encoding reach that path at all, which is why this surfaced on darkpool.
  The failure was invisible: the open error was logged and an empty frame returned. `read_silver` and `read_gold` now raise when files are present but cannot be opened, including when a partition's only artifact is a zero-byte part file — a lost write is not an empty partition. Genuinely absent data still reads as empty, as do partitions holding only AppleDouble sidecars or `.tmp` partial writes.
- **Darkpool reads no longer open every file in the feed** (`heber/features/pipelines/darkpool_features.py`): both Silver reads declared a `time_range` but never set `prune_by_dt`, so the walk visited all 1,486 files under `feed=darkpool` and read each footer regardless of the window asked for. Measured during the v2 regeneration: a 46-day slice spent 56m38s returning 12,179 rows, from 8 files' worth of window. `dt` is derived from `ts_event` on write, so pruning it over the range the scan filter already declares cannot drop a qualifying row.
- **The two Gold pipelines that died every night now survive the run** (`heber/features/pipelines/market_regime_features.py`, `darkpool_features.py`): both were OOM-killed on all three attempts nightly inside the 3 GB gold-poller, burning 91 and 14 minutes per run to produce nothing. `market_regime` read every equity ticker's bars over a ~127-day window (~6.4M rows) and discarded all but ~160k; it now reads a month at a time and reduces to daily closes before moving on. Chunk size matters in both directions — a per-day loop was measured at ~90s per call regardless of rows returned, because bars fragments mix `string` and `large_string` and every open re-reads all 3,322 fragment schemas, which would have traded the OOM for a timeout. `darkpool` was the only Gold pipeline with no column projection, batching or chunking — it read whole rows for the full universe over ~37 days, when only `ts_event`, `underlying`, `notional` and `premium` are used. `market_regime_features` has not been written since 2026-06-09 as a result.
- **The catalog coverage scan no longer holds a transaction open while it walks the disk** (`heber/catalog/seeds.py`, `heber/catalog/api.py`): the scan alternated work — walk a feed, upsert its rows, walk the next — on a session shared with dataset discovery, committing once at the end. The transaction opened by the first upsert then sat idle for the length of every later walk, against a Postgres `idle_in_transaction_session_timeout` of 5 minutes. 232 of 235 discovery passes failed in one container lifetime, always at an upsert, leaving coverage frozen since 2026-07-20. The walk now completes into an in-memory manifest before any statement runs, discovery and coverage use separate sessions, and each feed commits as it is written so a late failure keeps the feeds already recorded.
- **Tier 3 health checks survive a restart** (`heber/health_monitor/service.py`): the once-a-day marker lived only in memory, and this service restarted 59 times in a week, so every restart began the schema, statistical and ml-readiness sweep again from the top — across that week not one Tier 3 run was recorded as finishing. Completion is now recovered from the results already written to the dated partition, read once per process rather than on every 60s tick.
- **Stale catalog coverage is now reported instead of passing silently** (`heber/catalog/api.py`, `heber/ops/dataflow_health.py`, `heber/config.py`): `data_coverage` has been frozen since 2026-07-20 while `catalog_periodic_scan_error` fired 232 times in one container lifetime, and nothing said so — `/health` only runs `SELECT 1`, and the five-minute dataflow report's `catalog_health` check polls that same endpoint. A new `GET /health/coverage` reports the age of the newest coverage row and returns 503 past `HEBER_CATALOG_COVERAGE_MAX_AGE_SECONDS` (default 6h, generous against the 300s scan interval), and the dataflow report carries it as a `catalog_coverage` check. Coverage rows already carry `last_updated_ts`, rewritten on every successful scan, so their maximum is the completed-scan watermark — no new table or bookkeeping.
  Freshness is deliberately a separate route from `/health`: that one is the container healthcheck, gates `heber-consumer` and `heber-backfill-consumer` through `depends_on: service_healthy`, and drives watchdog restarts, so failing it on stale data would turn a data problem into a restart loop and could block ingest from starting.
- **The container supervisor now covers what an outage actually takes down** (`scripts/heber_docker_watchdog.sh`): it watched seven `heber-*` services, which left three gaps. It never saw `heber-backfill-consumer`. It never saw Heber's upstream, so when a Docker Desktop restart left `data-gateway` and `data-gateway-redis` down on 2026-08-12, the consumers crash-looped ~105 times each for 1h40m and recovery was manual — those two are now started by name, since they belong to a different compose project that Heber's `compose up` cannot reach, and upstream is brought back before Heber so recovery is not just a restarted crash-loop. And a `paused` container is neither running nor recoverable by `compose up`; containers have repeatedly been observed paused while still reporting `Up`, processing nothing, so they are now unpaused.
- **`vol_of_vol` reads daily bars only** (`heber/features/pipelines/market_regime_features.py`): the UVXY fallback scanned every bar interval to compute a statistic that reduces to one close per date. Measured over the production window (46 partitions, 2,768 files): all intervals returned 1,623 rows, `1Day` returned 46, and the scan ran 4.9x faster. The file count is unchanged — the saving comes from Parquet row-group statistics letting the scanner skip pages rather than decode them.
- **Compaction makes progress on both kinds of stuck partition** (`heber/writer/compactor.py`): a file larger than the whole batch byte budget used to be admitted alone — the budget loop always accepts its first candidate — after which the batch collapsed to one entry, was discarded, and left the partition untouched, so the next scan repeated it. 297 `massive_taq_*` partitions were capped that way every day for a week without a single merge. Oversized files are now excluded up front so the compactable remainder still merges. Separately, the batch was capped at 50 files regardless of size while the loop runs hourly, so a trading day landing ~274 files averaging 12 KB drained at 50/hour — `dt=2026-08-07` still held 273 uncompacted parts five days later, which is the main reason reads of recent data are slow. The byte budget is now the binding constraint and the file cap is a fan-out guard, so a normal day compacts in one pass.
- **Gold reads can prune by date too** (`heber/reader/core.py`, `market_regime_features.py`, `ticker_base_rates.py`): `read_gold` had no `prune_by_dt` at all, so every call walked the whole dataset — `momentum_features` is ~7,000 files across 88 dates, and that single read was measured at 25m27s on a cold run, the largest component of the market_regime pipeline once its Silver reads were scoped. `read_gold` now accepts the same opt-in flag, and `breadth_proxy` and `ticker_base_rates` use it. Because `write_gold` partitions on `date(ts_event)`, pruning a dataset timed by `ts_label` would drop partitions that could hold matching rows — that now raises instead of quietly returning a short result.
- **Reads that prune by date no longer walk the whole feed** (`heber/reader/core.py`): `_collect_parquet_files` recursively walked every partition directory under a feed on every read, ignoring `time_range` entirely — so reading one month of `bars` still visited all 2,580 `dt=` directories. On a cold page cache that walk was measured at 15m48s, paid once per read, and `market_regime` performs six. The walk now visits only the `dt=` partitions overlapping the range when `prune_by_dt=True`, which callers already set to declare the same range to the scan filter. Falls back to the full walk when a feed has no `dt=` partitions, so unpartitioned layouts are unaffected.
- **`market_regime` dispersion now reads daily bars only, and its values change for the production window** (`heber/features/pipelines/market_regime_features.py`, `heber/reader/core.py`): the pipeline read every bar interval and reduced afterwards — 1,986,898 rows for a single month against the 32,214 daily ones it needed, 62x — which is what pushed a cold-cache run to 2058s against a 1799s timeout. `read_silver` gains an opt-in `timeframe` filter pushed into the scan; it raises rather than silently returning every interval if the feed has no `timeframe` column.
  The deliberate cost is that a ticker-date with intraday bars but no daily bar no longer contributes a close. This **restores** what every Gold row written before 2026-06-09 was computed with — the old `len(bars) > 1_000_000` branch always fired on the production window and filtered to `1Day` — so the series stays comparable with its own history. It does differ from the handful of runs on the intermediate build that gap-filled. A gap-filled close is the last intraday print of the UTC day rather than an official close, so excluding it removes asynchronous-pricing noise from exactly the thinnest-data tickers. Orion consumes this dataset.
- **A missing Silver column no longer produces silently zeroed darkpool features** (`heber/features/pipelines/darkpool_features.py`): the reader intersects a requested projection with the dataset schema, so an absent column is dropped rather than raised, and the compute step read it via `.get(name, 0)` — yielding a complete set of zero-valued features. Required columns are now checked after each read and fail loudly.
- **Backfill runs no longer bury their own logs under a warning storm** (`heber/writer/consumer.py`, `heber/config.py`): the "stale timestamp on a live source" check skipped backfill by testing `envelope.source != "backfill"`, but `source` is the delivery method (`websocket`/`rest`) and the backfill driver emits `rest` — so the suppression never fired and every historical record logged a warning. One 2h16m backfill produced 493,403 warnings (95% of that container's log, ~73 MB/hour against a 250 MB cap), destroying the history you would need during an incident. The check now keys off stream identity via the new `HEBER_REDIS_BACKFILL_STREAM_NAME` setting. Live-stream staleness is still reported — that remains the poisoned-record signal.
- **UW units/warrants tickers stopped dead-lettering** (`heber/writer/key_normalization.py`): the `=`-suffix cleanup added earlier was unreachable. `normalize_envelope_for_silver` runs the strict `SYMBOL_PATTERN` first, which rejects `AAC=`/`PNAQ=`/`SAMO=` and yields `None`, while the lenient normalizer that strips `=` sat behind an `if symbol is not None` gate it could never satisfy. Those symbols now reach Silver. Tickers with a trailing `+` (`VAL+`) still dead-letter deliberately — the suffix has no documented meaning here, and quarantining beats silently rewriting a ticker. The DLQ record already names the rejected key, so they stay identifiable.
...
- **`scripts/run_gold_pipelines.sh`**: the host-cron Gold runner that wrote the wrong-schema `darkpool_features` files. Its crontab entry was already removed and the container gold-poller is the single scheduler, so the script was dead code that still worked if anyone ran it — and running it would have resumed the corruption. Two duplicate-scheduler incidents in one week made keeping it indefensible.
...
- **`heber_consumer_acked_total`** (`heber/ops/metrics.py`): the heartbeat proves the event loop is alive, not that it is making progress — a flush stuck retrying keeps ticking. The rate of this counter distinguishes "flushing slowly" from "wedged".

### Fixed

- **Acknowledging a batch no longer implies its events reached disk** (`heber/writer/consumer.py`): `_flush_layers` returned `True` whenever nothing raised, including when `flush_if_needed` found no partition due and flushed nothing — and the caller acknowledges on `True`. That was survivable only because the 30-second time trigger fired on every cycle; the moment a batch could be held back deliberately it would have acknowledged data that existed only in memory, losing it on restart. It now reports success only when nothing remains buffered.
- **Shutdown acknowledges before closing Redis** (`heber/writer/consumer.py`): `stop()` closed the connection first and `_final_flush()` ran after, so anything written but not yet acknowledged was left pending and redelivered on the next start — reprocessing it into Bronze, which has no deduplication. `_final_flush` is now async, flushes, acknowledges, and then closes.
- **The backfill consumer no longer reports unhealthy while working correctly** (`heber/writer/consumer.py`, `docker-compose.yml`): the liveness gauge ticked only at the top of the run loop and per message, never during the flush — which occupies its thread for minutes on the bind mount — so the healthcheck failed on essentially every batch. The gauge is now refreshed from the awaiting coroutine (a genuinely wedged loop still stops ticking), and the backfill service's staleness threshold moves 180s → 600s with a 300s start period. The live consumer keeps 180s.
- **Silver write metrics were double-counted** (`heber/writer/silver.py`): `write_silver_parquet` records the write and `SilverWriter._flush_partition` recorded it again for the same file, inflating `heber_writer_files_written_total`, `rows_written_total`, and `bytes_written_total` for Silver by 2x and skewing the flush-duration histogram. Removing the duplicate also removes two bind-mount round-trips per file, spent purely to recompute a size the callee had already measured.
- **Redundant `exists()` before `stat()` in the write path** (`heber/writer/{bronze,utils}.py`): each path lookup on the bind mount is an uncached round-trip costing ~2.4s, and the result was only used for a metric after the write had already succeeded. Now a single guarded `stat()`.

- **Documentation now states the correct `event_id` hash algorithm** (`PRD.md` §6.3, `docs/schemaaudit.md`, `docs/code-standards.md`, `docs/system-architecture.md`, `CLAUDE.md`, `AGENTS.md`, `heber/models/envelope.py`, `heber/models/silver.py`): every one of these described `event_id` as a SHA256 hash. Data-Gateway actually computes it as BLAKE2b-128 (`hashlib.blake2b(..., digest_size=16).hexdigest()`) over the pipe-joined `provider|feed|instrument_key|ts_event|*uniques`. Anyone producing events outside the gateway and following these docs would have generated non-matching `event_id`s, defeating Bronze-layer deduplication and silently landing duplicate rows. The `Decimal` stringification rule (`str(d)` → `1.50`, never `str(float(d))` → `1.5`) is now documented alongside it, since getting that wrong breaks dedup the same way.

- **The daily statistical audit of the `bars` feed runs again** (`heber/reader/core.py`): the health monitor's Tier-3 profiling check failed every day on `bars` with `ArrowNotImplementedError: Unsupported cast from string to null`, so the data-quality control on the primary equity feed had effectively been off. Out of 3613 `bars` Parquet files, one written on 2026-02-13 stored `timeframe` as an all-null column and others stored `quality_flags` as an empty list; both land on disk as Arrow "null" types that nothing else can be cast into. The reader now resolves those against the concrete type another file supplies, so a single such file no longer takes down reads of the whole feed. Reads of `bars`, and any other feed carrying a column that was entirely empty on some day, now return their rows instead of erroring.

  Two related read failures are fixed by the same change: columns that disagree on numeric type across files (`trade_count` is stored as both integer and floating point in `bars`) previously returned an **empty** result rather than an error, silently reporting a populated feed as having no data; and because the outcome depended on which file the scanner happened to inspect first, the same feed could read fine on one machine and fail on another.

- **The statistical audit now reads the whole trading day instead of a single instant** (`heber/health_monitor/checks/statistical.py`): it asked for a window running from a date to the *same* date, which both resolve to midnight, so only rows landing exactly on that instant could match. Every intraday feed — `bars` included — came back empty and was skipped as "no data" — no `statistical_*` result appears anywhere in the last six days of stored health output. The audit now covers the full day, and skips partitions outside it rather than scanning a feed's entire history to profile one day.

- **A feed the statistical audit cannot read is now reported instead of skipped** (`heber/health_monitor/checks/statistical.py`): an unreadable feed was logged and dropped, so it contributed no results at all — indistinguishable from a healthy feed in the health output, which is how this outage stayed invisible. Such a feed now produces an `ERROR` result naming the failure, and the remaining feeds still get audited.

- **The ML readiness checks now read the day they are auditing** (`heber/health_monitor/checks/ml_readiness.py`): the audit window was the trading date on both ends, and a bare date means midnight — so the read only ever matched rows stamped exactly at 00:00:00 UTC, which no market-hours row ever is. The zero-leakage audit, PSI label-drift check, feature-null check and cross-sectional completeness check were all reading empty results and reporting a clean pass every day. The window is now the whole UTC day. The leakage audit also named a dataset that does not exist (`alert_barrier_labels`), so the label side of the guardrail never ran at all; it now audits `labels_alert_barriers`.

- **Feature-null alerts now distinguish a broken writer from an upstream that was unavailable** (`heber/health_monitor/checks/ml_readiness.py`): with the checks live, every column populated by an outbound enrichment call — the Greeks, GEX/VEX, max pain, market tide, IV rank and the underlying-bars lookups — sits between 34% and 44% null in normal operation and would have raised 15 warnings on every single run. Those columns now report their coverage rate without alerting, and still export it to Prometheus. Everything else comes straight off the alert payload and still alerts above the 5% threshold, because a null there means the writer is broken. An enrichment column that is 95%+ null still alerts: that is the upstream returning nothing all day, not partial coverage.

- **The label-drift check now measures a label that exists** (`heber/health_monitor/checks/ml_readiness.py`): PSI was computed on a `label` column, which `labels_alert_barriers` does not have — so it skipped every day. It now tracks the barrier outcome (expired / hit_sl / hit_tp). Two further bugs in that check would have surfaced the moment it ran: the daily baseline was written without its key, so the rolling 30-day comparison would have stayed frozen on the first recorded day forever, and a baseline spanning more than one day crashed the check outright.

- **Gold reads no longer drop columns that were added after the dataset was created** (`heber/reader/core.py`): the reader took its schema from whichever partition happened to be first, so a dataset whose columns grew over time exposed only what its oldest partition had. On `meta_label_features` that hid `ts_event`, `ts_available` and `instrument_key` — present in 96 of 120 partitions — from every read. Worse, the predicates are only applied when their column is in the schema, so `asof_time`, `time_range` and `instrument_keys` were all silently ignored on that dataset: a point-in-time read returned rows from after its own cutoff. Gold reads now merge the schemas of all partitions whenever a column the read is actually filtering on is missing, so a predicate is never quietly dropped; the merge is skipped where it would change nothing, which keeps it off the datasets with thousands of partitions. Timestamp columns written at different resolutions widen to the finer one instead of failing the read, and a read that hits a type conflict part-way through now retries with merged schemas rather than reporting the dataset as empty. Fragments that disagree about the timezone of a point-in-time column are refused outright rather than guessed at.

- **A point-in-time read that cannot be filtered now fails instead of returning everything** (`heber/reader/core.py`): passing `asof_time` for a Gold dataset with no `ts_available` column used to drop the filter and return every row, which is the exact leakage the parameter exists to prevent. It now raises.

- **Quarantined rows no longer come back out of the reader** (`heber/reader/core.py`): `HeberReader` walked the entire dataset root, so the feature rows deliberately diverted into `_quarantine/` — rows whose enrichment returned no Greeks at all — were being read straight back into ML training inputs. The quarantine only ever kept them out of the `dt=` partitions, not out of reads. Any path under a `_quarantine` directory is now skipped. Expect Gold reads that previously picked up quarantined rows to return fewer rows. Because those rows no longer show up as "100% null Greeks" in the health report, a day whose feature rows were entirely quarantined now raises its own warning instead of reading as a quiet day with nothing to check.

- **Gold `meta_label_features` now writes `expiry` as one type everywhere** (`heber/ml/datasets.py`): producers handed the column dates, ISO strings and `YYYYMMDD` ints and each landed as-is, so the same column existed as `date32`, `string` and `int64` across different `dt=` partitions. A read of the whole dataset could not merge them, and now raises outright rather than silently coercing your dates into strings. Every write path — live watch enrichment, the enrichment backfill scanner, and quarantined rows — is coerced to a calendar date at the single point where the Parquet file is written, including partitions where every expiry is null. Values that are not a recognisable date become null and are logged with the raw value. **Existing partitions written before this change are not repaired** — reading the full dataset root still fails until they are rewritten.
- **A pipeline subprocess that fails before it can run now says why** (`heber/gold_poller/child.py`, `heber/gold_poller/service.py`): the child entrypoint imports config inside its functions rather than at module scope. Imported at module scope, a settings failure killed the child during `spawn` bootstrap — before any of our code ran — so the traceback reached only the child's stderr and the poller could report nothing but "died without a result", which reads identically to the OOM kill this isolation exists for. Such failures are now caught and reported over the result queue **with their traceback**, so the error names its own cause. Both this and the subprocess-weight fix depend on that bootstrap chain staying stdlib-only, so a test now asserts the property directly — one heavy import added to `child.py` or either package `__init__` would otherwise silently restore both the 143 MB bootstrap and the mute death. Tests in `tests/unit/test_gold_poller_isolation.py`.

- **Corrected the `slow` test marker's documentation** (`pyproject.toml`, `CLAUDE.md`, `AGENTS.md`, `docs/testing-guide.md`): all four claimed slow tests were "excluded by default", but `addopts` carries no `-m "not slow"` — they have always run on every commit. The docs now say so and name the flag to deselect them. Behaviour is unchanged: the marker covers only the gold-poller isolation tests (~4.5s), and excluding them would have removed the sole coverage of the subprocess-isolation guard from the gate.

- **Isolated Gold pipelines no longer drag the whole poller into their subprocess** (`heber/gold_poller/child.py`, `heber/gold_poller/service.py`): each pipeline runs in a `spawn` child, and `spawn` re-imports whichever module holds the process target before it can run anything. That target lived in `gold_poller/service.py`, so every child imported `HeberReader` and its pandas/pyarrow chain — **143 MB and ~0.6s of bootstrap** — before reaching the pipeline it was spawned for, on the one code path whose entire purpose is surviving memory exhaustion. The target now lives in a stdlib-only leaf module: **37 MB and ~0.06s**. This is also why `tests/unit/test_gold_poller_isolation.py` was intermittently red — its healthy and timeout tests need the child to actually survive, and on a loaded machine a 143 MB child sometimes did not.
- **A dead pipeline subprocess is now reported by what actually killed it** (`heber/gold_poller/service.py`): the poller claimed `likely OOM-killed` for *every* child that died without reporting, including children that exited with an ordinary Python error — so a broken import and a genuine out-of-memory kill produced identical, misleading log lines. It now distinguishes a signal death (`killed by signal 9`, the OOM case this isolation exists for) from the child's own interpreter exiting (`exited 1 before reporting`), and points at the child's stderr for the traceback.

- **A pipeline result delivered just as its subprocess exited is no longer discarded** (`heber/gold_poller/service.py`): the wait loop treated "child is gone" as "child produced nothing" without looking at the queue one final time, so a result flushed in the window between the polling `get()` timing out and the liveness check was thrown away and reported as a death. Narrow, but it turned a successful pipeline into a spurious failure and retry.
...
- **`quality_flags` provenance on Gold `meta_label_features`**: rows whose Greeks could not be reconstructed point-in-time now carry `greeks_no_point_in_time_source` (and `enrichment_skipped_stale`), so a training run can exclude or down-weight them instead of silently treating a stale value as real. Such rows are exempt from the all-Greeks-null quarantine guard — their Greeks are honestly absent rather than silently broken, and their remaining features are point-in-time correct — while unflagged null-Greek rows still quarantine exactly as before. `MetaLabelDatasetBuilder` **drops flagged rows from training by default** (`DatasetConfig.include_unrecoverable_greeks=False`), `quality_flags` is excluded from the model feature matrix, and the enrichment backfill scanner skips flagged rows instead of re-selecting them every cycle.

- **`heber_watch_enrichment_skipped_stale_total` metric**: counts alerts whose live-only enrichment the freshness gate refused, so the live path degrading into flagged rows during a long backlog drain is visible on a dashboard rather than only in logs.

- **Documented the permanent Greeks gap for 2026-08-06 and 2026-08-07** (`docs/data_contract.md`): no historical option-Greeks source exists (Alpaca's snapshot endpoint is live-only; the gateway's historical option routes return NBBO and OHLC only), and Heber's own `option_chain_snapshot` capture has no data for those two dates. Their feature rows are written with null Greeks and flagged rather than left missing.

- **`scripts/normalize_gold_expiry.py` — one-off repair for the mixed `expiry` types in Gold `meta_label_features`**: rewrites partitions written before the producer fix so the whole dataset can be read again. Dry-run by default; `--apply` does the rewrite. Only the `expiry` column is touched — every other column keeps its exact type and values — and each original is kept beside the new file as `*.pre-expiry-migration` so a rollback is a rename. Takes the same partition lock the live watch writer uses, refuses to touch any dataset other than `meta_label_features`, stops the whole run on an unreadable file, and skips partitions already in the right shape so it can be re-run safely. Sweeps the 120 canonical day-partitions only — the ~10k files in the quarantine tree are excluded from reads anyway, so rewriting them is dead work (`--include-quarantine` if you ever need it). Values that are not a recognisable date (including numbers that are not a whole `YYYYMMDD`) become null and are counted per file in the dry-run report rather than being guessed at.

- **`scripts/clear_silver_partition.py` — guarded delete-then-write recovery tool**: clears whole Silver day-partitions so a backfill can rewrite them cleanly. The compactor dedupes exact `event_id` collisions but cannot remove *stale* rows a re-backfill leaves behind under a different content hash — e.g. the contract-anonymous `oi_change` rows (`option_symbol` NULL) written before the named-backfill era (a provider change dropped `option_symbol` from the daily `oi_change` REST pull end of March 2026). Recovering such a day means dropping the partition, then republishing it. Guardrails refuse any path outside the Silver root, non-`dt=` leaves, and unsafe `feed`/`instrument_type` tokens; dry-run is the default and deletion requires `--apply`. Wiped Silver is recoverable from immutable Bronze. Tests in `tests/test_clear_silver_partition.py`.
- **Documented the permanent `iv_rank` / `iv_term_structure` gap for 2026-07-20 and 07-21** (`docs/data_contract.md`, new "Known Coverage Gaps" section): both feeds come from current-snapshot endpoints with no history API, so the two days evicted during the 07-20/07-21 EOD overload are permanently lost upstream and cannot be backfilled. Recorded so downstream consumers treat the gap as a source-data limitation, not a pipeline defect.
- **Nightly lakehouse backup — the first second copy** (`scripts/backup_lakehouse.sh`, LaunchAgent `com.empire.heber.backup`, daily 21:30 PT): rsync of Bronze/Silver/Gold + the catalog Postgres data dir to the 8TB `Raw Data` drive, without `--delete` (a broken source mount can never propagate deletions). Guards on the volume sentinel at the source and the target drive's presence; writes twin completion markers. New dataflow-health `backup_freshness` check (warns via Discord when the marker exceeds `HEBER_BACKUP_FRESHNESS_HOURS`, default 30). Before this, the entire lakehouse existed as a single copy on a removable exFAT drive that faulted twice in nine days.
- **Off-machine dead-man heartbeat** (`heber/ops/dataflow_health.py`, `HEBER_HEARTBEAT_URL`): dataflow-health pings an external check URL (healthchecks.io-style) after every cycle, `/fail` on a failing report. If pings stop — machine dead, volume gone, loop wedged — the external service alerts. Every existing monitor was colocated with the machine and died with it during the Jul 12–19 outage. Empty URL disables.
- **Deployment provenance in health reports** (`scripts/deploy.sh`, dataflow-health report field `deployed_sha`): deploy.sh records the deployed commit to `/data/ops/deployed_sha`; reports surface it (info-only — the #35 fix sat committed-but-undeployed for 9 days with nothing making that visible).
- **Mount-liveness and catalog checks in dataflow-health, with Discord alerting** (`heber/ops/dataflow_health.py`, `heber/config.py`, `docker-compose.yml`, `scripts/run_native_heber_service.sh`): during the 2026-07-11 volume drop + postgres death, dataflow-health reported `overall_status=ok` every 5 minutes for the whole weekend — it had no postgres/catalog check, no mount check, and market-closed mode skipped everything else. It now checks a sentinel file on the data volume (`.heber-sentinel` — created once on the real volume; disappears if the mount breaks or resolves to an empty placeholder) and the catalog `/health` endpoint (`HEBER_HEALTH_CATALOG_URL`), in **all** modes regardless of market hours, and dispatches failing checks to Discord via the existing debounced `DiscordNotifier`.
- **Future-dated events are dead-lettered instead of minting bogus partitions** (`heber/writer/consumer.py`, `heber/writer/ingest_contracts.py`): `ts_event` more than 1h in the future is rejected non-retryably to the DLQ (`ts_event_out_of_range`) before any Bronze write, since partition paths derive from it. A *live*-source event more than 30 days in the past logs a `stale_ts_event_live_source` warning (backfills legitimately carry old timestamps) — instrumentation for the recurring poisoned `dt=2023-06-23` crypto record.
- **`heber-backfill-consumer` — a dedicated consumer that decouples bulk backfill from the live stream** (`docker-compose.yml`, `scripts/deploy.sh`): the UW historical backfill (`ftds`/`greek_exposure`, etc.) shared the live `heber:events` stream with real-time feeds, so a multi-year flood filled the size-capped stream and evicted un-read **live** data — the root cause of the silent data loss seen the week of 2026-06-29. Data-Gateway's `uw_backfill_driver.py` now publishes to a dedicated `heber:events:backfill` stream, and this new service runs the same writer code against it (consumer group `heber-backfill-writers`, its own heartbeat healthcheck, metrics on host `127.0.0.1:9095`). The live and backfill streams now have independent MAXLEN caps and consumers, so a backfill flood can only ever trim re-runnable backfill data. Added to `deploy.sh`'s default service set. **Deploy this before pointing the driver at the backfill stream** (else backfill records land in a stream nobody reads and eventually trim). Pairs with the Data-Gateway change of the same name.
- **Self-healing EOD feed reconcile** (`heber/gold_poller/reconcile.py`, wired into `GoldFeaturePoller`): after the EOD deadline, if a daily UW feed's Silver partition never landed (e.g. evicted from the capped Redis stream while the consumer was down — the 2026-06-25 `oi_change` loss), the poller re-pulls it from UnusualWhales via the Data-Gateway backfill API so the gap heals from source instead of staying lost. Off by default (`HEBER_EOD_RECONCILE_ENABLED`); runs once per trading day at `HEBER_EOD_RECONCILE_HOUR`:`MINUTE` ET (default 17:45, after the feeds' EOD deadline so in-flight data isn't mistaken for a gap); targets `oi_change, iv_rank, historic_option_volume` (`iv_term_structure` excluded — its provider method is snapshot-only and can't backfill a past date); a read error is treated as "present" so a read blip never triggers a spurious backfill.
- **A supervisor for stopped Heber containers** (`scripts/heber_docker_watchdog.sh`, `launchd/com.empire.heber.docker-watchdog.plist`): a launchd job reconciles every 2 minutes and `docker compose up -d`s any app container found stopped or missing. Compose `restart: always` only restarts containers that *crash* — a container left *stopped* (e.g. across a deploy, as happened on 2026-06-25 when heber-consumer sat down for ~3h through the EOD publish) had nothing watching it. The watchdog only touches compose when something is actually down, so steady-state is a cheap inspect. Load it once with `launchctl load -w launchd/com.empire.heber.docker-watchdog.plist`.
- **heber-consumer has a real liveness healthcheck** (`docker-compose.yml`, `heber/writer/consumer.py`, `heber/ops/metrics.py`): a new `heber_consumer_loop_heartbeat_unixtime` gauge is stamped at the top of every run-loop iteration (fresh even on idle no-data cycles, unlike last-write), and the container healthcheck fails if it goes stale for 180s — catching a stalled-but-running consumer and making `deploy.sh`'s health wait meaningful (it previously treated the disabled healthcheck as instantly healthy).
- **Silver salvage tests now verify the salvage actually happened**: three tests for `write_silver_parquet`'s row-by-row salvage path (`tests/test_writer_coverage.py`, `tests/test_exception_hardening.py`) previously asserted nothing, swallowed every exception, or used all-valid input that never triggered salvage. They now feed a genuinely un-coercible row and assert the valid rows are persisted while the bad row is dropped.
...
- **Dedupe-store benchmark** (`scripts/debug/bench_dedupe_store.py`): measures sync vs pipelined Redis registration overhead against the <5% enablement gate.

### Changed

- **Live consumer reads 5000 events per batch (was 2000)** (`docker-compose.yml`): during the final trading hour on 2026-07-22 the Alpaca SIP quotes feed pushed 4k+ events/s into `heber:events` while the consumer drained ~2–3.5k/s, growing lag ~1.3k/s toward the 500k trim edge. The consumer's read→process→flush→ack cycle pays its flush cost once per batch, so the config-max batch of 5000 (already used by the backfill consumer) amortizes that cost over 2.5× more events. Mitigation, not cure — the quotes firehose sharing a stream with low-volume feeds (flow_alerts, darkpool) is the structural issue.

- **`batch_processed` log now splits process time from flush time** (`heber/writer/consumer.py`): each consumer iteration log gains `process_seconds` (CPU: parse/validate/buffer) and `flush_seconds` (bind-mount gzip/parquet I/O) alongside the existing `elapsed_seconds`/`messages_per_second`. This attributes the live-mix throughput ceiling to the half that actually caps it — the two are fixed differently (more processing parallelism vs faster/overlapped flush I/O), and the choice between scaling out consumers and optimizing the single-process loop depends on which dominates. No behavior change.
- **Writers and consumers are now safe to run as multiple sharded containers on one Redis group** (`heber/writer/bronze.py`, `heber/writer/silver.py`, `heber/writer/consumer.py`): each Bronze/Silver writer now stamps a per-process id into its output filenames (`events-{ts}-{id}.jsonl.gz`, `part-{ts}-{id}.parquet`) and each consumer gets a uuid-suffixed group-consumer name. Previously two processes writing the same partition in the same microsecond generated identical filenames and could clobber each other's `.tmp` staging file (partial-write corruption on a live partition), and two consumers starting in the same second shared one group identity. This is the prerequisite for horizontally scaling the live `heber:events` consumer past its single-process throughput ceiling (~225–770 msg/s) by running several `heber-consumer` containers on the `heber-writers` group — Redis delivers each message to exactly one consumer, so the split is safe once names and filenames are unique. It also retroactively hardens the existing live-vs-backfill shared-volume writes. Matches the uuid-suffix convention already used by the watch and Gold writers. Tests in `tests/test_writer_shard_safety.py`.
- **Bronze/Silver partition flushes run concurrently to keep the consumer ahead of backfill floods** (`heber/writer/utils.py`, `heber/writer/bronze.py`, `heber/writer/silver.py`, `heber/config.py`): a UW historical backfill (e.g. `ftds`) scatters one Redis batch across hundreds of date partitions, and the writers flushed them **one at a time** — thousands of tiny gzip/Parquet writes per cycle to the slow macOS bind mount. That capped consumer drain throughput far below inflow, so the size-capped `heber:events` stream stayed pinned at its cap and evicted un-consumed **live** feeds (bars/trades/flow_alerts/darkpool) — silent data loss observed across the last week. Due partitions in a flush cycle now write in parallel (new `HEBER_WRITER_FLUSH_MAX_WORKERS`, default 8; set to 1 for the old serial behavior). Each partition is a distinct file so writes never contend, and the safety contract is unchanged: a partition is dropped from the buffer only after its write succeeds, a failed write keeps its records for redelivery, and the first error is re-raised so the batch is not ACKed. This raises drain throughput but does not by itself remove the root cause — bulk backfill still shares the live stream (see the follow-up to route it to a dedicated stream). Tests in `tests/test_writer_flush_concurrency.py`.
- **Post-EOD self-heal reconcile is now on by default** (`heber/config.py`): `HEBER_EOD_RECONCILE_ENABLED` defaults to `True`, so a daily UW feed whose Silver partition never landed is automatically re-pulled from the gateway backfill API after the EOD deadline. Safe to enable now that the gateway backfill supports these feeds and retries transient glitches; the reconcile only fires on a genuine gap (past 17:45 ET, read errors treated as "present"), runs once per trading day, and no-ops gracefully without a gateway API key. Set `HEBER_EOD_RECONCILE_ENABLED=false` to opt out.
- **`deploy.sh` refuses to rebuild during the UW EOD ingest window** (`scripts/deploy.sh`): a rebuild recreates heber-consumer, and if that lands across the ~16:31 ET EOD publish the consumer is down while the ~114k-message burst arrives and the capped Redis stream evicts the unread feeds — exactly the 2026-06-25 `oi_change` loss. Deploys are now blocked 16:25–16:45 ET unless `--force` / `HEBER_DEPLOY_FORCE=1`.
- **Exact-match Redis dedupe verified against the live event bus and kept off by default**: a synchronous Redis SET per event adds ~2-6ms (170-680% overhead at average ingest) and even pipelined batching degrades to ~0.4ms/event under bus contention — neither clears the 5% throughput gate at peak. The measured numbers live in the `HEBER_DEDUPE_REDIS_ENABLED` field description and `.env.example`; Bloom saturation fail-open remains the active false-positive protection.
- **CI now blocks on security and type gates**: bandit (high severity/confidence) re-enabled in pre-commit and as a blocking CI step; mypy runs as a blocking CI step over the strict module list (`reader`, `config`, `core`, `universe`, `utils`).
- **Test-suite wall-clock audit**: all 101 `datetime.now()`/`date.today()` uses in tests classified; the single boundary-fragile site (watch poller perf test — crashed on Feb 29, produced future timestamps at midnight UTC) fixed with calendar-safe offsets.
...
- **DLQ Redis stream is now bounded** (`heber/config.py`, `heber/writer/consumer.py`, `heber/watch/consumer.py`): both the writer and watch consumers now apply an approximate `MAXLEN` cap (`HEBER_REDIS_DLQ_MAX_STREAM_LEN`, default 100k) when enqueueing to `heber:events:dlq`. The DLQ lives on a cache-mode Redis with LRU/TTL eviction, so the previously-unbounded stream churned its reported length into the millions purely from eviction — while only a handful of distinct events actually failed per day. The durable on-disk fallback files remain the audit record of truth.
- **Consumer flushes Bronze/Silver off the event loop** (`heber/writer/consumer.py`): the gzip/Parquet flush now runs via `asyncio.to_thread`, so a large batch flush no longer blocks the event loop and starves the consumer's Prometheus metrics endpoint and Redis heartbeats (a contributor to `dataflow_metrics_fetch_failed` read timeouts). Flush-before-ACK ordering is unchanged, and the flush never overlaps buffer writes; this is a responsiveness fix, not an ingest-throughput change.

### Fixed

- **EOD reconcile backfill can now actually reach the Data-Gateway** (`docker-compose.yml`, `heber-gold-poller`): the poller runs on a different compose network than the gateway, so its reconcile pointed at `http://localhost:8080` — unreachable from inside the container — and carried no gateway API key, so every self-healing backfill POST failed (`ConnectError`/401) the moment a daily feed went missing (observed 2026-07-20: `oi_change`, `iv_rank`, `historic_option_volume` all lost and un-recoverable). The poller now reaches the gateway over the host bridge (`host.docker.internal`) and sends the same key `heber-watch` uses (from `.env`), matching the working watch-service pattern.
- **`iv_rank` dropped from the EOD reconcile targets** (`heber/config.py`, `docker-compose.yml`): like `iv_term_structure`, `iv_rank`'s UW provider method is snapshot-only — a re-pull returns the *current* value stamped now, so it lands under today's partition instead of reconstructing the missing date (confirmed 2026-07-20: a back-dated re-pull wrote a `dt=2026-07-22` partition, the gap stayed). Reconcile now targets only `oi_change, historic_option_volume`, which back-date correctly. Set as both the config default and a poller env override so it takes effect before the next image rebuild.
- **Nightly Gold-pipeline cron no longer dies with `uv: command not found`** (`scripts/run_gold_pipelines.sh`): cron runs with a minimal `PATH` that omits `~/.local/bin` where `uv` lives, so the 2 AM job failed instantly every night (observed Jul 20–21, zero pipelines run). The script now prepends `~/.local/bin` to `PATH` at the top. The job backfills a trailing 7-day window each run, so the next successful night self-heals the missed dates.
- **Equity Gold features no longer vanish on dates with sparse daily bars** (`heber/features/pipelines/equity_features.py`): the memory guard dropped ALL intraday bars whenever any `1Day` bars existed, so dates with minute bars but little/no daily coverage (1Day bars run 24–238/day vs a ~500-symbol universe; the whole Jul 13–17 backfill) produced zero momentum/volatility/label rows. It now drops only intraday rows whose `(instrument, date)` is already covered by a daily bar, and resamples the surviving intraday in per-instrument chunks so the sort/groupby working set stays bounded.
- **Catalog row counts include hourly sub-partitions** (`heber/catalog/seeds.py`): `_count_parquet_rows` only globbed `*.parquet` directly inside a `dt=` directory, so hourly-partitioned feeds (e.g. `quotes` with `hour=13/…` subdirs) reported 0 rows in coverage records. It now walks recursively while still skipping macOS `._` sidecar files and directories. This was the one still-relevant piece of the stale `fix/heber-silver-stall-and-lag-alerts` branch, salvaged before deleting it.
- **Test suite is green again after the backup dead-man and P2-hygiene changes** (`tests/test_dataflow_health.py`, `tests/test_compactor_safety.py`, `tests/test_sdk_catalog_defaults.py`, `tests/test_logging_level_filtering.py`): report-level dataflow-health tests now stub the new `backup_freshness` check (with two new direct unit tests covering it), the compactor resource-fork test creates the mount sentinel its tmp data root was missing, the SDK defaults test expects the corrected `redis://localhost:6379`, and the level-filtering tests drain the empire-core log queue before asserting on captured output.
...
- **Quarantined bogus `dt=2023-06-23` crypto partitions** (data operation, no code): 1,612 Bronze files + 10 Silver parquet files produced by the delisted-MATIC poisoned record moved to `ops/quarantine/misdated-matic-20260719/` with a README (Bronze is immutable — relocated, not deleted). Source fix landed in Data-Gateway (`DEFAULT_CRYPTO_PAIRS`).
- **Catalog `/health` actually exercises the database** (`heber/catalog/api.py`): it returned a static 200, so docker kept the container "healthy" through 46.8 hours of 100% API failure while postgres was dead (2026-07-11 → 07-13). It now runs `SELECT 1` and returns 503 when the DB is unreachable, so the container healthcheck, the watchdog, and the new dataflow-health catalog check all see the truth.
- **Compactor fails loudly on a broken mount instead of reporting an empty lakehouse** (`heber/writer/compactor.py`): with the volume unmounted it logged `Compaction cycle complete, partitions_scanned: 0` as if healthy; it now raises when the volume sentinel is missing, which surfaces as a compactor error every cycle.
- **The native service runner fails loudly when dependency sync breaks instead of relaunch-looping silently** (`scripts/run_native_heber_service.sh`): each service ran `exec uv run …`, whose implicit `uv sync` swallowed its exit code — so a broken `uv.lock` (e.g. an unresolved `<<<<<<< HEAD` merge conflict, seen breaking the `alert-check` job last week) just exited and relaunched every `StartInterval`, burying the parse error. The runner now runs `uv sync` explicitly up front and, on failure, emits a distinct greppable `HEBER_RUNNER_ERROR: …` line and exits — no behavior change on the healthy path (`uv run` synced anyway). Also corrected a stale `heber/config.py` comment that still described `eod_reconcile_enabled` as "Off by default" (it defaults to `True`).
- **A Redis health-probe timeout no longer fabricates a phantom "Consumer group is missing" critical** (`heber/ops/dataflow_health.py`): a 2s socket timeout (docker-proxy jitter) made `_collect_redis_signals_once` return `group_exists=False`, which flipped **both** `redis_connection` **and** `redis_consumer_group` to critical in the same cycle — 8 spurious "group missing" pages last week, though Redis never restarted and the group's `entries-read` never reset. When the connection isn't OK, `redis_consumer_group` is now reported **skipped** ("Redis unreachable") instead of "missing"; `redis_connection` still carries the real unreachable signal, and a genuinely-absent group on a reachable Redis still fails. Tests in `tests/test_dataflow_health.py`.
- **The pending-recovery drain no longer stalls the consumer's liveness heartbeat** (`heber/writer/consumer.py`): `_recover_pending_batch` flushed Bronze/Silver with a **synchronous** `_flush_layers()` call on the event loop, and recovery can loop up to `_MAX_RECOVERY_BATCHES` (100) times — so a large drain blocked the loop past the 180s healthcheck window and flapped the container `unhealthy` (observed this week). The flush is now offloaded via `asyncio.to_thread`, matching the normal consume path; flush-before-ACK ordering is unchanged.
- **Consumer recovery reclaims a full pending backlog and keeps running, not just one batch at startup** (`heber/writer/consumer.py`, `heber/config.py`): `_recover_pending_messages()` claimed a single `redis_claim_batch_size` (100) batch and ran only once at startup, so a consumer that died holding a large pending backlog stranded the rest forever — observed today as ~1,800 of 1,900 `ftds` messages left unreclaimed by the replacement consumer. Recovery now drains in batches until the idle-pending set is empty, and the running consumer re-runs recovery every `HEBER_REDIS_RECOVER_INTERVAL_SECONDS` (default 300s, 0 disables) so messages stranded by a sibling consumer that dies mid-session are reclaimed without waiting for a restart. Surfaced while investigating a same-day `oi_change` EOD data loss (the consumer was down during the publish window and the capped `heber:events` stream evicted the unread EOD feeds — a separate upstream issue).

- **Critical-feed Discord alarm no longer spams false "feed appears dark" alerts** (`heber/ops/notifier.py`, `heber/health_monitor/feed_registry.py`, `heber/config.py`): two amplifiers were turning a healthy lakehouse into an evening of Discord noise — the `darkpool` feed alone fired 72 CRITICALs in the logged history. (1) The notifier alerted on the *first* failing liveness cycle, so any single-cycle blip — a transient `/Volumes/heber` bind-mount read error counted as "0 rows", or a deadline-edge daily-feed check — fired CRITICAL and then RECOVERED minutes later (two messages per blip). A new `HEBER_ALERT_DEBOUNCE_CYCLES` (default 2) requires that many consecutive failing cycles before the first alert and silently resets on recovery, so blips never reach Discord; a genuinely sustained outage still alerts (~one extra cycle, ≈5 min, later) and keeps its hourly cooldown reminders. (2) `darkpool` delivery is batched, not steady — after the close the feed legitimately goes quiet for ~2h before the next batch lands, which a 60-minute lookback read as "dark" and flapped all evening. Its lookback widened to 180 minutes to span those inter-batch gaps; a truly dead feed still trips after 3h. The dominant evening offender (`darkpool`, then the `oi_change`/`greek_exposure` deadline edges) goes quiet while real outages — like today's genuine `oi_change` EOD miss — still alert.

- **Triple-barrier label availability resolves on trading sessions, not calendar days** (`heber/gold/labels.py`): `compute_availability_time` (the PRD §29.3 `write_label` writer) computed `ts_available` for daily `forward_window`s by adding calendar days and snapping to a hardcoded 16:05 ET. A Friday `close_to_close` 1d label was marked queryable on Saturday 16:05 — up to ~3 days before its value is observable at the next session's close (e.g. the Tuesday after MLK Monday) — violating the zero-leakage contract, and it ignored 13:00 ET early-close half-days. Daily windows now advance N trading sessions via `MarketCalendar` and snap to that session's real `session_close()` (weekends, holidays, and half-days handled), with the result returned in the input timezone; intraday (`Nh`/`Nm`) and week/month windows keep wall-clock arithmetic. The unused `market_close_time` parameter was removed. No live callers were affected (the gold poller uses `equity_features`, whose calendar-day leak was already fixed); tests and PRD §29.3 updated.

- **The zero-leakage audit no longer reports a clean PASS when it could not run** (`heber/health_monitor/checks/ml_readiness.py`): a failed Gold read was swallowed to an empty DataFrame and the leakage audit treated empty as a PASS, so a corrupt/unreadable `alert_barrier_labels` / `meta_label_features` dataset would be reported leak-free on the system's primary look-ahead-bias guardrail. `_safe_read_gold` now returns `None` on read failure (vs an empty frame for genuine no-rows) and logs at WARNING; the leakage audit emits an `ERROR` (not PASS) when the read fails. The sibling drift/null check (`statistical.py`) likewise raised its silently-dropped-feed log from DEBUG to WARNING so an unreadable feed is visible at the default INFO level. Tests in `tests/health_monitor/test_ml_readiness.py`.
- **Daily cross-feed completeness check finds Silver data** (`heber/ops/daily_health.py`): `_check_cross_feed_completeness` read `feed={feed}/dt={dt}`, a path that never exists (dt nests under `instrument_type=`), so it always saw fewer than two feeds and permanently reported `Only 0 feed(s) have data; cannot compare` instead of comparing instrument coverage. It now globs `instrument_type=*/dt={dt}` and unions symbols across instrument types. The test helper now writes the real layout.
- **Tier-2 partition and volume checks resolve Silver partitions under `instrument_type=`** (`heber/health_monitor/checks/partition.py`, `heber/health_monitor/checks/volume.py`): both looked for `feed={feed}/dt={dt}`, but the writer always nests dt one level deeper (`feed={feed}/instrument_type={type}/dt={dt}`). The top-level path never exists, so the partition check would emit a P0_CRITICAL `Missing dt= partition` for *every* Silver feed on every run, and the volume-drop check silently skipped every feed (and wrote no baselines, self-starving its own alarm). Both now glob `instrument_type=*/dt={dt}` and aggregate across instrument types. The test fixtures, which wrote the same wrong path and masked the bug, now mirror the real layout. Tests in `tests/health_monitor/`.
- **Exported label templates no longer leak across weekends/holidays** (`heber/features/templates/labels.py`): `compute_return_labels` and `compute_classification_labels` (re-exported from `heber.features`) had the same calendar-vs-trading-day `ts_available` bug as the pipeline labels — they stamped availability `horizon` *calendar* days after `bar_start_ts` while the return resolved `horizon` *trading* bars ahead. Availability is now derived from the resolving bar (`bar_start_ts.shift(-horizon) + 1 day`). The two template tests, which only checked column presence on gap-free daily data, now assert availability on weekend-spanning business-day data.
- **Forward-return labels no longer leak across weekends/holidays** (`heber/features/pipelines/equity_features.py`): `compute_return_labels` computed the forward return over `horizon` *trading*-day bars but stamped `ts_available = ts_event + horizon *calendar* days`, so every label spanning a weekend or holiday became queryable before its resolving close was observable — look-ahead bias that `HeberReader.read_asof` would have surfaced into Orion backtests/training (e.g. a Friday 1-day label was available Saturday, before Monday's resolving close). `ts_available` is now derived from the actual resolving bar (`horizon` trading bars ahead), conservatively one day past its date so the resolving 16:00 ET close is fully observable in both DST regimes; it stays tz-aware UTC. Regression tests in `tests/test_return_label_availability.py`.
- **Watch DTE is computed against the ET market date** (`heber/watch/consumer.py`): `_calculate_dte` used `date.today()` (UTC/local), so any alert processed in the evening ET (past UTC midnight) got a days-to-expiry one day too low, mis-bucketing options near expiry. It now uses the `America/New_York` date. Test in `tests/test_watch_dte_et.py`.
- **Writer buffers no longer leak dead partition keys** (`heber/writer/bronze.py`, `heber/writer/silver.py`): after flushing a partition both writers reset the buffer to an empty list but kept the key. Since partition keys embed `dt=`/`hour=`, a long-running consumer accumulated one dead empty-list entry per `(provider/feed,instrument_type,dt,hour)` forever — a slow unbounded memory growth. The flushed key is now deleted (the `defaultdict` recreates it on the next write); a flush failure still retains the buffered records for redelivery. Regression test in `tests/test_writer_coverage.py`.
- **Liveness checks no longer fire false `feed appears dark` criticals on early-close days** (`heber/health_monitor/checks/liveness.py`): continuous-feed windows are hardcoded to a 16:00/20:00 ET close, so on half-days (13:00 ET close — e.g. day after Thanksgiving, Christmas Eve) every regular-hours feed produced P0 criticals all afternoon once it legitimately went quiet. Each window end now shifts earlier by the early-close delta (from the calendar's actual session close), preserving each feed's after-hours tail relative to the real close (darkpool's +4h tail → 17:00 on a 13:00-close day). Regression tests (early-close afternoon suppressed, normal-day afternoon still alarms) in `tests/health_monitor/test_liveness.py`.
- **Catalog `/api/v1/health/summary` no longer stalls the API event loop** (`heber/catalog/api.py`): the handler read and concatenated every health-check parquet partition (>1000 small files/day on the bind mount; a single-day read measured 50–80s) directly on the event loop, freezing the single-worker catalog API and its background discovery task for the whole read. The blocking read/aggregate now runs in a worker thread via `asyncio.to_thread`. Test (event-loop-responsiveness) in `tests/test_catalog_health_summary_offload.py`.
- **Gold poller no longer leaves the just-closed session one trading day stale** (`heber/gold_poller/service.py`): the EOD trigger fires at 16:35 ET, but `_run_all_pipelines` computed the target day with a helper hardcoded to a 17:00 ET cutoff, so any run in the 16:35–16:59 window targeted *yesterday* — and because the run marks itself done for the day, the corrective 17:00 run never happened. Today's Gold features only appeared on the next day's run. The poller now computes features for today's session (which `_should_run` has already validated is a completed trading day past the configured trigger). Test in `tests/unit/test_gold_poller_eod_date.py`.
- **Gold poller result accounting now uses real Gold dataset keys** (`heber/features/pipelines/{gex_regime_features,market_tide_context_features,market_regime_features}.py`): composite single-output pipelines previously returned internal component names (`net_gex`, `market_sentiment_score`, `_merged`, etc.) at the top level, so the poller logged fake dataset names and over-counted rows. They now return exactly the registered Gold dataset key with component stats nested under `components`; `tests/test_pipeline_result_contract.py` now enforces the registry contract.
- **Non-dev Postgres credential guard parses the actual URL password** (`heber/config.py`): the guard now percent-decodes and compares the password field exactly, closing a URL-encoded default-password bypass and avoiding false positives for legitimate passwords that merely contain the dev password text. Tests in `tests/test_config_credential_guard.py`.
- **Naive ISO timestamps are now stamped UTC during Silver coercion** (`heber/writer/normalizer.py`): `_coerce_timestamp` returned a naive datetime for ISO-8601 strings with no UTC offset (e.g. `"2026-01-02T03:04:05"`), letting a naive `ts_event`/`ts_available` slip into Silver and break point-in-time comparisons. Naive results are now stamped UTC and aware results converted to UTC, matching `_parse_string_timestamp`. Test in `tests/test_normalizer_coercion.py`.
- **DLQ fallback backlog scan no longer fails on macOS sidecar files** (`heber/writer/dlq_fallback.py`): the startup backlog count aborted with `dlq_fallback_scan_failed` (EPERM) whenever an AppleDouble `._*.json` sidecar existed in today's partition on a bind-mounted volume, hiding the real backlog count from operators. Sidecars are now filtered by name before any `stat()` call — same treatment `HeberReader` already applies to Parquet scans. Test in `tests/test_dlq_fallback.py`.
- **Gold label reads work again** (`heber/reader/core.py`): `read_label()` returned an empty DataFrame for every dataset because `read_parquet_dataset()` silently yielded no files when given a direct path to a single Parquet file (the macOS-sidecar-filtering file walk only handled directory roots). Single-file paths are now read correctly, restoring the label read/write round-trip.
- **Watch feature extractor's gateway response cache is now LRU-bounded** (`heber/watch/features.py`): entries were only evicted lazily on a cache hit after TTL expiry, so a long-running heber-watch accumulated one entry per distinct endpoint/symbol/params combination indefinitely (audit F-8). The cache is now an LRU capped at 4,096 entries (constructor-tunable). Tests in `tests/test_watch_response_cache_bound.py`.
- **Darkpool liveness no longer false-alarms pre-market** (`heber/health_monitor/feed_registry.py`): the darkpool feed's monitoring window started at 04:00 ET, but darkpool prints only begin at the open — so the check fired `darkpool: 0 rows in last 60m — feed appears dark/degraded` ~100×/pre-market-hour every trading day. The window now starts at 09:30 ET and keeps the after-hours tail to 20:00 (darkpool legitimately flows until ~19:00), so genuine intraday/after-hours outages still page while the pre-market false positives are gone. Regression tests (pre-market suppressed, after-hours still monitored) in `tests/health_monitor/test_liveness.py`.
- **DLQ size alert now reflects real failures, not Redis bookkeeping** (`heber/ops/dataflow_health.py`): `dlq_size_exceeded` warned off the eviction-perturbed Redis `XLEN`, which oscillated into the millions on a handful of genuinely-failed events. The check now counts today's durable DLQ audit files (the dedup-by-event-id record of truth) and escalates warn/critical at daily-rate thresholds, so the alert tracks the real failure rate. Tests in `tests/test_dataflow_health.py`.
- **A single transient Redis blip no longer flips two critical health checks** (`heber/ops/dataflow_health.py`): a 2s connect/socket timeout during a Redis restart momentarily failed both `redis_connection` and `redis_consumer_group` for an entire report cycle. The signal collector now retries once before reporting, absorbing the blip without tolerating a genuinely-down Redis for long. Tests in `tests/test_dataflow_health.py`.
- **A single slow metrics scrape no longer logs a `dataflow_metrics_fetch_failed` traceback** (`heber/ops/dataflow_health.py`): the consumer/watch Prometheus endpoints intermittently take longer than the 5s client timeout to respond when the consumer event loop is briefly busy, producing a `ReadTimeout` that recovers on the very next request. `_fetch_metrics_samples` now retries once (mirroring the Redis-signal retry) so a transient blip is absorbed; only a genuinely-unreachable endpoint (both attempts fail) is logged — and without the identical-every-time httpx traceback, which was pure noise. Freshness still falls back to filesystem mtime when both attempts fail. Tests in `tests/test_dataflow_health.py`.
- **Dataflow-health launchd runner stops spamming `mkdir: Permission denied`** (`scripts/run_native_heber_service.sh`): the health-report directory was created before the data-volume mount was verified, so every launchd cycle logged `mkdir: /Volumes/heber: Permission denied` whenever the volume was unmounted. The mount guard now runs first; the report dir is created only once the volume is present (which also prevents split-braining the lakehouse onto the boot disk).
- **Bloom-filter dedupe can no longer silently drop legitimate events** (`heber/ops/reliability.py`, `heber/writer/consumer.py`): the consumer's deduplicator ran Bloom-only, so a false positive was treated as a duplicate — the event was ACKed with status "dropped" and never reached Bronze (audit F-6). Three layers now prevent this: (1) a `RedisDedupeStore` backs the Bloom filter with exact `EXISTS` checks — Bloom-negative events never touch Redis, Bloom hits are verified before dropping; (2) the Bloom filter is sized 256M bits for the observed ~105M events/day and fails OPEN (`bloom_saturated_fail_open`) when its estimated false-positive rate exceeds 1%, instead of degrading into mass drops; (3) the store itself fails open on Redis errors — a duplicate write is recoverable, a dropped event is not. New settings `HEBER_DEDUPE_REDIS_ENABLED` (default off — registration issues a synchronous Redis SET per event, so evaluate throughput before enabling at full stream volume) and `HEBER_DEDUPE_REDIS_TTL_SECONDS`; new metrics `heber_consumer_dedupe_saturated_passes_total` and `heber_consumer_dedupe_store_errors_total`. Tests in `tests/test_dedupe_backing_store.py` and `tests/test_dedupe_bloom_saturation.py`; root `conftest.py` disables the Redis store in tests so they never touch the live event bus.

### Removed

- **Dead `DeadLetterQueue`/`DLQEvent` classes removed** (`heber/ops/reliability.py`): the in-memory DLQ class was used by nothing except its own test — the live dead-letter path is the Redis DLQ stream plus the disk-first fallback in `heber/writer/dlq_fallback.py`. Keeping two DLQ implementations misled readers about which one runs.
...
- **`heber/backtest/` removed**: backtesting is owned by the trading-system repos (Nemesis, Atlasv2, etc.), not the lakehouse — nothing in Heber or its scripts imported the module. The zero-leakage primitives backtests rely on (`ts_available` predicate pushdown, `read_asof`, `asof_join`) live in `heber/reader/` and are unchanged.
- **Schema-registry subsystem removed** (`heber/schema_registry/`, `python-schema-registry-client` dependency, `HEBER_SCHEMA_REGISTRY_*` settings, `apicurio-registry` docker-compose service, `scripts/init-databases.sh`): per the 2026-06-10 audit decision, the registry client was never imported by the writer, reader, or any live service — schema enforcement happens via Arrow schemas in `heber/schemas/silver.py`. The package-level import had been broken for months without anything noticing, which is its own evidence of deadness. The auxiliary-database init script created databases only for removed services and is gone with them.
...
- **Typed Silver for UnusualWhales company financial statements** (`income_statement`, `balance_sheet`, `cash_flow`): promoted the three fundamentals feeds from Bronze-only to typed Silver so 20+ years of quarterly **and** annual fundamentals become queryable via `HeberReader.read_silver(...)`. A single UW call per ticker returns ~102 rows (both report types) spanning 2005→present. Columns are the **raw UW line items verbatim** (verified against the live endpoint 2026-07-03) — ~25 income-statement fields (total_revenue, ebit, ebitda, operating_income, net_income, …), ~37 balance-sheet fields (total_assets, total_shareholder_equity, cash_and_cash_equivalents, retained_earnings, goodwill, …), and ~30 cash-flow fields (operating_cashflow, cashflow_from_financing, capital_expenditures, dividend_payout, …), all typed float64. The only rename is `fiscal_date_ending` → `fiscal_date` (the period-identity anchor + sole required field); derived fields (FCF, ratios) belong in Gold. Added Arrow schemas (`heber/schemas/silver.py`), Pydantic record classes (`heber/models/silver.py`), routing (removed from `BRONZE_ONLY_SILVER_DATASETS`, added to `CONTRACTED_RAW_FEEDS` + `FEED_ALIASES` so both the snake_case and hyphenated `/api/stock/{ticker}/balance-sheets` REST-path names route to Silver), and catalog seeds. Verified end-to-end against live AAPL data: 102/102 rows route to Silver, distinct fiscal-period event_ids, zero drops. Regression tests in `tests/test_ingest_feed_contract_matrix.py` (`test_financial_statement_feeds_route_to_silver_not_bronze_only`, `test_financial_statement_payload_keys_map_to_silver_columns`). NOTE: the legacy `BalanceSheet`/`IncomeStatement`/`CashFlow` dataclasses in `heber/schemas/additional.py` are a minimal Bronze-era subset that did **not** match the live payload (they carry phantom `eps`/`free_cash_flow`/`fiscal_quarter` fields UW doesn't return and omit ~35 real line items); the Silver schema mirrors the real endpoint instead. `ts_available` for backfilled rows reflects ingest time (Heber's standard for all driver-backfilled history); order/gate by `ts_event` = the fiscal period.
