...
- **Daily label availability now snaps to New York market close** (`heber/gold/labels.py`): UTC `ts_label` inputs were snapped to `16:05 UTC` instead of `16:05 America/New_York`, making daily forward labels visible several hours early. Availability is now computed in market time and converted back to the input timezone. Tests in `tests/test_label_availability_math.py`.
