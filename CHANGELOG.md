...
- **Reading a Silver feed no longer re-derives its schema on every call** (`heber/reader/core.py`): Silver fragments persist `feed` and `instrument_type` as data columns that duplicate the hive partition keys, and the compactor writes them as `large_string` where the real-time writer wrote `string`. Hive partitioning has to reconcile the two, so `ds.dataset()` fails and the reader falls back to opening every fragment's schema individually — a fixed cost per read that does not scale with rows returned, measured at 1-5 minutes on `feed=bars/instrument_type=equity` (2,599 fragments) with a cold page cache. `market_regime` reads that path five times per run and paid it five times. The unified schema is now cached per file set, so it is derived once. The cache is keyed on each file's name, size and mtime rather than the path, so a new fragment, a compaction, or an in-place replacement all invalidate it — `scripts/force_dedupe.py` and the Massive bars promotion both rewrite Silver files at their existing names. It holds one entry per path, so a long-running poller cannot leak, and an entry that stops opening is dropped rather than turning the feed permanently empty.
...
- **`market_regime` dispersion values change for the window the pipeline actually runs on** (`heber/features/pipelines/market_regime_features.py`): the old code filtered to `timeframe == "1Day"` only when a read exceeded 1M rows, which the production window always did — so every ticker-date that had intraday bars but no daily bar was dropped. Below the threshold the same code kept them. Results therefore depended on how much data a run happened to load. The gap-fill is now applied consistently, which means the new dispersion series is **not directly comparable to values written before 2026-06-09**. A gap-filled close is the last intraday print of the UTC day rather than an official close, so it carries more asynchronous-pricing noise for exactly the tickers with the thinnest data. Orion consumes this dataset.
