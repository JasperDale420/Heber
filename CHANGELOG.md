...
- **The catalog coverage scan no longer takes longer than the alarm watching it** (`heber/catalog/seeds.py`, `heber/config.py`): one pass took **8h34m** — the container started at 21:34:30 and `coverage_scan_complete` landed at 06:08:43 — against a `HEBER_CATALOG_COVERAGE_MAX_AGE_SECONDS` of 6h. A pass that takes longer than the staleness threshold can never satisfy it, so `/health/coverage` was going to report stale between every pass and alert on a healthy system, and a container restart guaranteed 8½ hours of it. The alarm was correct; the thing it measured was too slow.
  Timing each phase showed the database was not involved: writing all 16,681 coverage rows took **6.6 seconds**. The entire cost was the disk walk, and on the exFAT lakehouse mount that cost is per-open latency — measured at ~28ms to open a directory and ~106ms to read one Parquet footer — not bandwidth or CPU. Three things followed from that. The walk visited every partition twice, once to find `dt=` directories and again to list the Parquet files inside each; it now makes a single pass. Directory listings and footer reads are issued concurrently rather than one at a time, since latency overlaps where CPU would not: `feed=bars` reads 4,146 footers in 16.7s instead of 440s. And zero-byte Parquet files are skipped by size instead of being opened — 175 of them exist, 173 under `feed=quotes`, and each one cost an open, a raised `ArrowInvalid`, and a full stack trace rendered into a JSON log line on every pass. The compactor and `HeberReader` already skipped empty files this way.
