...
- **The underlying condition is not fixed by any of this**: `feed=quotes` having 825k un-compacted part files is a lakehouse health problem in its own right — every `HeberReader` read of that feed pays for it too. `heber-compactor` has been running continuously on `massive_taq_quotes_1s` and never reaches `feed=quotes`.
