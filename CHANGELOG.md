...
- **`expiry` coercion no longer fabricates a real-looking date from a malformed value** (`heber/ml/datasets.py`, `scripts/normalize_gold_expiry.py`): `coerce_expiry_to_date` truncated anything to its first 8 digits, so `20260320.5` and `"2026-03-20junk"` silently became `2026-03-20` — a wrong value with nothing distinguishing it from a clean one. It now requires an exact `YYYY-MM-DD`, `YYYYMMDD`, or whole-number match and returns `None` (logged) otherwise, same as an unparseable value already did. The migration script's separate, already-strict `_coerce_for_migration` is gone — the shared function is now strict everywhere, so both write paths reject the same malformed inputs.
- **The live watch path no longer truncates a malformed alert `expiry` into a fake clean date** (`heber/watch/consumer.py`): `_extract_and_store_features` built its `FlowAlertRecord` with `str(alert["expiry"])[:10]`, which slices to the first 10 characters and parses whatever is there rather than validating the whole value — `"2026-02-20junk"` became a clean-looking `2026-02-20`, the exact fabrication failure mode the `coerce_expiry_to_date` fix above exists to close, except upstream of it: by the time that function saw the value it was already a plain `date` object, so the stricter check never got a chance to reject it. Found by the adversarial review of that fix, which specifically traced the live production path rather than trusting the producer survey. Alert expiry now goes through the same shared `coerce_expiry_to_date`; a rejected value raises before the record is built, so the alert is skipped and logged like any other feature-extraction failure instead of silently mislabeled.
- **`meta_label_features` writes now fsync before publishing, closing a zero-byte-file window on the exFAT lakehouse volume** (`heber/ml/datasets.py`, `scripts/normalize_gold_expiry.py`): both the live watch-consumer write path (`_atomic_write_parquet`) and the expiry-migration script (`_rewrite_expiry`) promoted their staged file with a bare `os.replace`. On exFAT — no journaling — a rename makes the filename visible immediately while the bytes can still be sitting in the page cache, so an unclean shutdown between the rename and the actual flush can leave a zero-byte file at that path, exactly the class of corruption `heber/writer/utils.py`'s Silver/Bronze writers already guard against. `_atomic_write_parquet` now publishes through `publish_parquet_atomically` — fsync plus a Parquet-footer and row-count readback, since a truncated-but-nonempty write would otherwise still land as a broken fragment that fails the whole dataset scan — and flushes the destination directory afterward, because this path overwrites an existing merged partition rather than only ever adding a fresh file. `_rewrite_expiry` copies the original aside as a backup immediately before replacing it; `shutil.copy2` doesn't fsync, so that backup's bytes and directory entry are now flushed durable *before* the replace runs (a crash right after the replace could otherwise leave the replace done and the backup itself missing or truncated), and the replace's own directory entry is flushed afterward too. Both temp-file cleanup paths were also restructured so a failure during cleanup can no longer mask the original publish error that triggered it.

  This closes the same class of gap the reader/schema fix above and #50 track, using the exact primitives Bronze/Silver/the compactor already trust — but an adversarial review of this fix surfaced that those shared primitives (`publish_file_atomically`/`fsync_directory`) rely on plain `fsync()`, which on Darwin does not guarantee the drive itself — especially external/USB-attached storage, how `/Volumes/heber` is mounted — has actually flushed its write cache; only `fcntl(fd, F_FULLFSYNC)` gives that guarantee (the same reason SQLite's macOS backend uses it instead of bare `fsync`). That gap predates this change and affects every caller of these primitives, not just the two here, so closing it is tracked separately rather than folded into this fix: [#55](https://github.com/JasperDale420/Heber/issues/55).
